---
title: "Lesson: Score-based diffusion via SDEs, the unifying view"
description: Lesson 11 derived denoising score matching as a noise-prediction MSE. Lesson 12 derived the DDPM training loss as a noise-prediction MSE from a Markov-chain ELBO. They look like the same equation. This lesson shows they are the same equation, formally, via the continuous-time stochastic differential equation (SDE) perspective. The forward chain is a discretization of a forward SDE; the reverse chain is a discretization of a reverse SDE; the noise predictor is the score function up to a fixed scaling. The equivalence unlocks a deterministic ODE sampler and a tractable likelihood evaluation.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
