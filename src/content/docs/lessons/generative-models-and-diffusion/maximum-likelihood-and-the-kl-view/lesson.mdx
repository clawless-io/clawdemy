---
title: "Lesson: Maximum likelihood and the KL view"
description: The previous lesson minimized the negative log-likelihood without saying why. This lesson derives it. Maximum likelihood turns out to be exactly the empirical version of minimizing the forward KL divergence from the data distribution to the model, and that single derivation explains why every likelihood-based paradigm (autoregressive, flow, VAE-via-ELBO) shares this objective.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
