...
Six trips around the loop, two different tools, one self-correction when a conflict showed up. No single step is clever. The power is in the repetition: each pass starts from a richer picture than the last, and the agent keeps going until the goal is actually done, not just described.
