...
That is the whole mechanism. Steps 2 through 4 are exactly Lesson 1's perceive-decide-act loop seen up close, with the perceive happening when the model reads the tool result (step 4).
...
- **Those steps are Lesson 1's perceive-decide-act loop seen up close.** Tool use is the mechanism that makes the loop able to act.
