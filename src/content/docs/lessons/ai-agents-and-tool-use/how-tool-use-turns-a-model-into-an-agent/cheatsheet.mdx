...
Steps 2 to 4 are Lesson 1's decide-act-observe loop seen up close.
...
LOOP  -> runs it -> { high: 58F, low: 47F, condition: "rain" }    (step 3)
MODEL -> "Tomorrow in Seattle: rain, high 58F, low 47F."          (step 4)
```
...
```
MODEL -> get_weather { city: "Seettle" } -> ERROR "unknown city"
MODEL -> get_weather { city: "Seattle" } (corrected) -> { rain, 58F }
```

An agent that never sees its failures cannot correct them.
