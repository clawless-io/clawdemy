...
```
A. runs get_weather("Seattle", "tomorrow") and returns { rain, 58F }
B. emits: call get_weather { city: "Seattle", day: "tomorrow" }
C. sends the model the tool menu: get_weather(city, day) "forecast..."
D. reads { rain, 58F } and replies "Tomorrow in Seattle: rain, 58F."
```
...
The point: the error came back as a step-3 result, the model read it, corrected the code, and called again. Same four steps, just run twice. If the loop had not fed the error back, the model would have been stuck or guessed.
