...
- **The name.** A short handle like `get_weather`. The model uses it to refer to the tool when it calls.
...
Take a single tool and write it two ways.
...
With the weak version, the model might pass `"tomorrow"` to a tool that expects `2026-05-21`, and the tool fails or misbehaves. With the strong version, the model knows the exact format and that it must resolve relative dates itself first. You have moved a whole class of failure from runtime into the definition.
...
Watch it work. The agent has only the `search_internal_docs` tool, and the user asks a general-knowledge question:
...
WITHOUT negative guidance:
  ...
  LOOP  -> {} (nothing; the knowledge base has no world facts)
...
The definition's fourth part, the expected output, matters for the same reason the description does: the model has to read the result and decide what to do next (Lesson 2's step 4). A tool that returns an opaque blob makes that hard. Compare two return shapes for the same weather tool:
...
- **Naming a tool vaguely.** `search`, `process`, `handle`, and `do_task` tell the model nothing. The name is the first and shortest description; spend it well.
