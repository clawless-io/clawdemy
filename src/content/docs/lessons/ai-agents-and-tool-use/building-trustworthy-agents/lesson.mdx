...
description: "Why a capable agent is not yet a trustworthy one: the six ways agents fail on their own, the guardrail that contains each, and matching it to blast radius."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
