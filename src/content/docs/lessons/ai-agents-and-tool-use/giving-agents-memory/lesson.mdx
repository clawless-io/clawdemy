...
Recall the loop from Lesson 1. The agent perceives the current state, decides, acts, and repeats, all within a single run toward a single goal. Everything it works with lives in that run. When the run ends, the working state is discarded. There is no place information persists unless you build one.
