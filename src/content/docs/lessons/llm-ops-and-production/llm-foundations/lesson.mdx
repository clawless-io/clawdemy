...
- **Sampling is controllable.** Two arguments you will see on every provider's API:
  ...
  - **`top_p`** (nucleus sampling) limits sampling to the smallest set of tokens whose cumulative probability reaches `p`. A common combination is moderate temperature + `top_p` around 0.9 or 1.0.
...
- **The output is also bounded by context.** `max_tokens` for the response is part of the same window; a model with a 200K context will not give you a 200K response on top of a 200K prompt.
...
- **Generation is the more expensive side.** Capping `max_tokens` and prompting for concise responses are the largest cost levers most applications have.
...
Two numbers matter to a user: **time-to-first-token (TTFT)**, the wall-clock from request to the first streamed token, and **tokens-per-second**, the streaming rate after that. Total response time is approximately `TTFT + (output_tokens / tokens_per_second)`. Practical consequences:
...
- **Generation is autoregressive:** sample one token from a probability distribution, append, repeat, until an end token, `max_tokens`, or a stop sequence stops it. **Output streams one token at a time**, which is why streaming is possible.
- **`temperature` and `top_p` control sampling.** Lower temperature = more deterministic; higher temperature = more varied. `top_p` (nucleus) limits sampling to the smallest set whose probability sums to `p`. Defaults usually suffice.
- **Context length is the hard input limit.** System + user + retrieved context + few-shot + history + `max_tokens` all share one budget. Hitting it forces tighter prompts, better retrieval, or a longer-context model.
