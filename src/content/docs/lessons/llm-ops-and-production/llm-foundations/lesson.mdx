...
description: "What a hosted LLM is at the API level, how it generates text token by token, and the three limits (context length, cost per token, latency) it lives under."
sidebar:
  order: 2
  label: Lesson
artifact: lesson
