---
title: "Agents"
description: "Lesson 10 of Track 21. What an LLM agent is (a tool-use loop with the model deciding the next step), the common patterns (ReAct, plan-and-execute, function-calling agents), the engineering failure modes that make agents hard to ship (loops, wrong paths, compound cost, harder evaluation), when to reach for an agent versus a single call, and how lesson 7's LLMOps discipline scales to multi-step loops. Mirror of the Full Stack Deep Learning Bootcamp agents session (Harrison Chase, LangChain). Taught technical-primer: WHAT, WHEN, WHAT-GOES-WRONG, HOW; agent-autonomy and contested-alignment debates explicitly out of scope."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
<ReadAlongDim
  src="https://audio.clawdemy.org/lessons/agents-lesson.mp3"
  slug="agents"
...
3. **Log every step, not just the final answer.** The lesson 7 log schema applies, plus the tool that was called, its arguments, a summary of the result, the step number, and the total steps so far. Without trajectory-level logs, post-mortems on agent failures are impossible.
