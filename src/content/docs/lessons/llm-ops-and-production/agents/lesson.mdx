...
2. **Cap iterations and identical calls.** Every production agent should have a max-steps limit (typically 6 to 12) and a "no identical tool call twice in a row" guard. The cap should error visibly, not silently truncate; surface it to the model when reasonable so it can adjust.
