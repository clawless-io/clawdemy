---
title: "Lesson: Training your own LLM"
description: The deep dive on the fine-tune point of the build-vs-buy spectrum. When training your own model is the right move, the staged pipeline most teams should follow (start from an open checkpoint, fine-tune, optionally preference-tune), the practical tools and economics, and the production-side trade-offs that decide whether the project pays back.
sidebar:
  order: 2
  label: Lesson
artifact: lesson
...
- **Match the format to the task.** SFT (supervised fine-tuning) data is `(instruction, response)` pairs in the model's chat template. Get the format right before scaling up data volume.
