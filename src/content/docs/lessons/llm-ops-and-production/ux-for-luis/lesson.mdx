---
title: "Lesson: UX for language user interfaces"
description: A language user interface is a new interaction surface, and the patterns that make one usable are different from the patterns of forms and buttons. Streaming, citations, regeneration, hedging, recoverable failure, and the supporting details that turn a working API call into an application a user actually wants to talk to.
sidebar:
  order: 2
  label: Lesson
artifact: lesson
...
The implementation is small (`stream=True` or a `messages.stream()` context manager in most clients) but the UX implications are larger:
