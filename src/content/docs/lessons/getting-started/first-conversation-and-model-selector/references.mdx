...
description: Where this lesson's facts come from in the Clawless knowledge base, plus the small set of outside sources worth knowing about model selection and chat ergonomics.
artifact: references
sidebar:
  order: 6
  label: References
...
- **["Which model should I use?"](https://www.anthropic.com/research/claude-4-5-sonnet)** Anthropic's own framing of when Sonnet vs Haiku vs Opus is the right call. The article is provider-specific but the *shape* of the question (capability vs speed vs cost) generalizes to every provider in the Clawless picker. Read it once, then make your own version of the decision tree as you use different models on real work.
