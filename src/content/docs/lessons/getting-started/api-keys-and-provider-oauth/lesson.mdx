...
description: "How Clawless Computer connects to AI providers. What an API key actually is, where yours lives, and the OAuth path that lets ChatGPT subscribers skip per-token charges."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
You paste it into [Clawless](https://clawless.ai) once. Clawless tucks it away in your operating system's secure storage (the same place your other saved passwords live) and uses it on every message you send. The key never appears on screen again once it is saved.
