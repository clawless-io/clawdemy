...
You start with a corpus of documents. Some are short. Many are long. A model's context window is finite, and embedding models work better on chunks that represent a single coherent idea than on whole documents. So the first job is to split documents into **chunks**, typically 200 to 800 tokens each.
...
Each chunk goes through an embedding model (the same kind of model from lesson 3) and becomes a vector of a few hundred to a few thousand floating-point numbers. These vectors get stored in a **vector database** (FAISS, Pinecone, Weaviate, Chroma, and others), indexed for fast nearest-neighbor lookup.
...
The reason this matters: you can identify which stage is broken by which symptom shows up. Chasing better generation when the real problem is retrieval is the most common wasted RAG effort.

## RAG versus fine-tuning

Both RAG and fine-tuning (lesson 7) shape what the model produces. They are different tools.

**Use RAG when** the answer depends on facts that change, when you need to cite sources, when the corpus is too large to fit in pretraining, when the corpus is private, or when you need to be able to add or remove individual documents on the fly.

**Use fine-tuning when** you want a behavior or style to persist without paying token cost on every call, when the change is about how the model writes rather than what facts it knows, or when you need consistency across many calls without managing a retrieval pipeline.

In real systems they are often combined. Fine-tune the model on your domain's writing style or response shape; use RAG to pull in the specific facts each query needs. The two address different problems and stack cleanly.
...
The canonical attack is straightforward. A malicious actor publishes a webpage, slack message, support ticket, PDF, or any other document type that gets ingested into a RAG index. The document contains text shaped like an instruction:
...
When a user asks a question whose retrieval pulls in this chunk, the model sees the injected instruction in its context. It has no robust way to tell that this instruction was not from the operator. It may follow it. The user gets the attacker's chosen response instead of an answer.
