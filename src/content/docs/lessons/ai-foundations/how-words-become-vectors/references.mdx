...
- **Multi-head attention, the full transformer block.** The next two lessons in this Stanford-adapted course. Each one builds on embeddings: multi-head attention runs the lesson 1 mechanism in parallel many times per layer over the embeddings; the transformer block wraps attention with feed-forward networks and normalization, all operating on the same dense vectors that embeddings produce.
