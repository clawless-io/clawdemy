...
**byT5** (byte-level T5): no tokenizer; vocabulary fixed at 256 entries (one per byte value); every character representable in at most two bytes. Trade-off: the model handles any text without a tokenizer.
...
Vanilla T5 (the original). mT5 (multilingual T5: broader training data and vocabulary). byT5 (byte-level T5: no tokenizer; vocabulary fixed at 256 entries since every byte value gets one; every character representable in at most two bytes).
