---
title: "Lesson: How transformers turn input into output: encoder-decoder and T5's span corruption"
description: "A walk through the T5 family of encoder-decoder transformers and the span corruption pretraining objective that distinguishes them from the next-token-prediction crowd. We start with the encoder-decoder architecture from the original 2017 transformer, walk the T5 / mT5 / byT5 lineage, build the span corruption mechanism through the lecture's teddy-bear example, and close on the lecturer's framing of why decoder-only architectures eventually dominated despite T5's strengths."
sidebar:
  order: 2
  label: Lesson
artifact: lesson
...
<AudioPlayer src="https://audio.clawdemy.org/lessons/encoder-decoder-and-t5-span-corruption-lesson.mp3" approxMinutes={18} />
...
**byT5** is the byte-level T5. Instead of using a learned tokenizer (the lecturer cites ~30k vocab as the contrast point), byT5 operates directly at the byte level. The vocabulary is fixed at 256 entries, one per possible byte value, and every character is representable in at most two bytes. byT5 can therefore handle any text without a tokenizer.
...
- **T5 (Text-to-Text Transfer Transformer) kept the encoder-decoder shape and changed the pretraining objective.** Vanilla T5, mT5 (multilingual), byT5 (byte-level, vocab size 256, no tokenizer).
