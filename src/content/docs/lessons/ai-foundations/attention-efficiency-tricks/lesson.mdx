---
title: "Lesson: How transformers scale to real-world data: sliding windows and KV-cache savings"
description: "A walk through two distinct efficiency problems and the two distinct fixes that show up in modern transformers. Standard self-attention is O(n^2) in sequence length, and the KV cache that speeds up decoding gets large quickly. Sliding window attention solves the first; sharing key and value projections across heads (MHA, MQA, GQA) solves the second. Two problems, two fixes, often combined in the same model."
sidebar:
  order: 2
  label: Lesson
artifact: lesson
...
<ReadAlongLesson src="https://audio.clawdemy.org/lessons/attention-efficiency-tricks-lesson.mp3"
  slug="attention-efficiency-tricks" approxMinutes={22} />
