---
title: "Lesson: Position embeddings: from sinusoidal to RoPE"
...
- **Position-embedding choice is one of the few places the field actually moved on.** Most of the original transformer architecture is intact in modern LLMs. Position embeddings, normalization placement, and attention head sharing are the parts that genuinely changed. If you read about "modern transformer tweaks" elsewhere, those three are usually the list (and we'll cover the other two in the next lessons).
...
- **RoPE rotates the query and key vectors.** The dot product of rotated vectors depends on relative position. Cheap to compute, no learned parameters, extends to any sequence length, has long-term decay built in. Most modern LLMs use it. Long-context extensions usually mean RoPE scaling.
