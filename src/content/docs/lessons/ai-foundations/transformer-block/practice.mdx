...
**2. Where in a transformer is position encoding added, and why is it not added at every block?**

<details>
<summary>Show answer</summary>

It is added **once**, to the embeddings, before the first block runs. After that, the position information is carried through every block via the residual stream. There is no need to re-add it at each block; doing so would be redundant work.
...
- **Input Embedding.** Looks up each token's dense vector from `W_E` (embedding lookup, lesson 3). Turns integer IDs into vectors the model can do math on.
...
Position encoding (added once before the first block), feed-forward network (per-token nonlinearity), residual connections (gradient flow + information preservation), and layer normalization (activation stability).
...
<Flashcard question="Where in the architecture is position encoding added?">

Once, to the embeddings, before the first block runs. The position information is then carried through every block via the residual stream; it is not re-added at each block.
