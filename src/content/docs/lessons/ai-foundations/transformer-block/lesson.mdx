...
<AudioPlayer src="https://audio.clawdemy.org/lessons/transformer-block-lesson.mp3" approxMinutes={25} />
...
Attention is a fundamentally **linear operation in the values** (it is a weighted sum of value vectors, with the weights being a softmax of dot products). Mixed with multi-head and the various projection matrices, it does a lot, but it leaves three structural gaps:
...
2. **No nonlinearity per token.** Without something nonlinear in each layer, stacking attention layers would collapse into one big linear transformation. The model would be no more expressive than a single attention.
...
Each of the four wrapping pieces fixes one of those gaps (or in the case of residuals, does double duty). The block design from the original paper is the smallest set of additions that turns attention into something a transformer can actually be built from.
...
- Position encoding is added **once**, to the embeddings, before the first block runs. It is not added at every block.
- Without position encoding, the transformer would be order-blind. With it, attention learns to use position as part of "who matters to me."
...
Two reasons the FFN earns its place in every block:

- **It adds nonlinearity.** This is the gap from the previous section: attention is linear in values, so without nonlinearity, stacking layers compresses into one big linear transformation. The activation function in the FFN breaks the linearity, which is what gives the transformer the power to model complex functions.
- **It transforms each token independently.** Attention does the cross-token mixing; the FFN does the per-token transformation. The two are deliberately complementary: attention "for each token, who else matters?" and FFN "for each token, what is its updated representation?"

In a typical model, the FFN parameters are roughly two thirds of the per-block parameter count. It is bigger than attention, which surprises most readers who have just learned attention and assume that is where the parameters live. The model is doing real work in both places.
...
Putting all four pieces together gives you the block diagram from the original paper.

<TransformerBlock />

What happens to one token's representation as it passes through one block:

1. Input vector enters at `d_model`.
2. Multi-head attention runs (cross-token mixing).
3. The attention output is added to the input (residual), then passed through LayerNorm.
4. Feed-forward network runs (per-token transformation).
5. The FFN output is added to the result of step 3 (residual), then passed through LayerNorm.
...
- **Modern variants compose from these pieces.** Almost every named modern transformer variant is a recombination of choices on each of these axes: which position encoding (sinusoidal, learned, RoPE, ALiBi), which normalization (LayerNorm, RMSNorm, DeepNorm), which FFN activation (ReLU, GELU, SwiGLU), and head-sharing (full multi-head, MQA, GQA from the multi-head attention lesson). One more axis worth knowing: where LayerNorm sits relative to the sub-layer. The original paper applied it after each sub-layer (Post-LN); modern implementations almost always apply it before (Pre-LN), which trains more stably at scale. Once you know the four boxes, every variant is a substitution at one of them.
...
**Thinking position encoding is added per block.** It is added once, before the first block. Subsequent blocks see the position information through the residual stream, not through new injections.
...
- **The four wrapping pieces.** Position encoding (added once before the first block), feed-forward network (per-token nonlinearity), residual connections (gradient flow and information preservation), layer normalization (activation stability).
...
- **Modern variants substitute on these axes.** RMSNorm vs LayerNorm, RoPE vs sinusoidal, SwiGLU vs ReLU, Pre-LN vs Post-LN, MQA or GQA vs full multi-head. Once you know the four boxes, the variants are easy to read.
