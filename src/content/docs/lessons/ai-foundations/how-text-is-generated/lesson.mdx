...
<AudioPlayer src="https://audio.clawdemy.org/lessons/how-text-is-generated-lesson.mp3" approxMinutes={22} />
...
Logits are not yet probabilities. To turn them into probabilities, apply **softmax** (the same operation from lesson 1, applied here across the vocabulary instead of across keys). Now you have a probability distribution: `vocab_size` numbers, all between 0 and 1, summing to 1.0. Each one is the model's predicted probability that this specific token comes next.
...
The first forward pass (over the full prompt) is slow. Every subsequent pass is roughly constant-time per new token, because the per-position work for prior tokens is reused.

This is why streaming responses appear at a roughly constant rate after a brief initial delay. The first-token wait is the model "processing your prompt"; the subsequent tokens stream because each one is now cheap.
...
- **KV caching makes generation roughly constant-time per token after the first forward pass.** Without it, generation would be quadratic in sequence length.
