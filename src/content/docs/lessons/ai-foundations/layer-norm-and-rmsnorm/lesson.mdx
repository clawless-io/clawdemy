---
title: "Lesson: Why transformers need stability to learn: LayerNorm, pre-norm, and RMSNorm"
description: A walk through what those small "Add & Norm" boxes in the original transformer diagram actually do, and the two ways the field has changed them since 2017. We start with the mechanism (subtract the mean, divide by the standard deviation, learn a rescale and a shift), name the LayerNorm-vs-BatchNorm distinction, walk the structural shift from post-norm to pre-norm, and end on RMSNorm as the modern simplification.
sidebar:
  order: 2
  label: Lesson
artifact: lesson
...
<AudioPlayer src="https://audio.clawdemy.org/lessons/layer-norm-and-rmsnorm-lesson.mp3" approxMinutes={18} />
...
The fix is to **bring the values of the components into a controlled range** before they reach the next sub-layer. The keyword in the literature is **internal covariate shift**: the distribution of activations shifts as the network trains, and that shift makes the next layer's job harder.
