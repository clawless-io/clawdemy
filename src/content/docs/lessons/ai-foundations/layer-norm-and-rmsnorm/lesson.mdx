...
The mechanism the original paper used is called **LayerNorm**. The 2017 transformer wrapped it around each sub-layer as `LayerNorm(x + SubLayer(x))`. Modern transformers do two things differently. First, they move the LayerNorm to a different place: `x + SubLayer(LayerNorm(x))` instead of around the residual. Second, they often use a simpler version of LayerNorm called **RMSNorm**. Both changes show up plainly in modern model cards. You will see "Pre-LayerNorm" or "Pre-RMSNorm" listed as architecture features. Each one exists for a specific structural payoff.
...
The original BatchNorm paper (Ioffe and Szegedy, 2015) framed the benefit as fixing **internal covariate shift**. The idea: the spread of activations shifts as the network trains, and that shift was supposed to make the next layer's job harder. That framing has not held up. Santurkar, Tsipras, Ilyas and Madry (2018, "How Does Batch Normalization Help Optimization?") showed that BatchNorm does not actually reduce internal covariate shift. They even injected extra shift after the normalization, and optimization was fine. The explanation accepted today is different. Normalization smooths the loss landscape, which lets gradient descent take larger, stable steps. The observation holds up well: deep networks train faster and more reliably with normalization. The mechanism is just more subtle than the original paper claimed.

LayerNorm itself, introduced by Ba, Kiros and Hinton (2016) for recurrent networks, had a different motivation from the start. Its framing is per-example stabilization. You normalize each example's activations using statistics from that example alone. This avoids the train-test gaps and small-batch failures that BatchNorm runs into. That motivation carried over cleanly to transformers, where each token's activation vector is normalized on its own, apart from the rest of the batch.
...
Second, BatchNorm has a structural weakness for sequence models. It depends on the batch composition. At training time you compute the mean and standard deviation of each component across the batch. At inference time the batch may be a single sequence, so those statistics differ. LayerNorm sidesteps that train-vs-inference gap entirely. It works per-token and never looks at other vectors in the batch.
