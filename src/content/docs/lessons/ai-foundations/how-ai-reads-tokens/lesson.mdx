...
AI can write a poem about strawberries. But ask it how many Rs are in the word, and it might fail.
...
The dominant algorithm for building this kind of vocabulary is **byte-pair encoding**, abbreviated BPE. GPT-2, GPT-3, GPT-4, GPT-5, and most other modern LLMs use a variant of it. The next section walks through what BPE does, and works one merge step by hand, so the mechanism stops being a black box.
...
The `</w>` is an end-of-word marker that BPE tracks so it knows where words end. The number in parentheses is how often each word appears.

**Step one: count every adjacent pair.** The pair `(e, r)` appears in `lower`, `newer`, and `wider`, weighted by their frequencies: 2 + 6 + 3 = 11 occurrences. The pair `(l, o)` appears in `low` and `lower`: 5 + 2 = 7 occurrences. Other pairs are smaller. The most frequent pair is `(e, r)` with 11.

**Step two: merge it.** Add a new token `er` to the vocabulary. Rewrite the corpus by replacing every `(e, r)` with the new token:

```
l o w </w>           (5 occurrences)
l o w er </w>        (2 occurrences)
n e w er </w>        (6 occurrences)
w i d er </w>        (3 occurrences)
n e w </w>           (1 occurrence)
```

That is one BPE merge. The vocabulary just grew by one entry (`er`). Run the same procedure again and the next most frequent pair might be `(er, </w>)` with 11 occurrences, so you would add `er</w>` as a new token, meaning "the suffix -er at the end of a word." Run it 50,000 times and you have your full BPE vocabulary.
...
If you paste "strawberry" into a real tokenizer (we will do exactly this in the practice section), it breaks into one or two tokens depending on which model's tokenizer you are looking at. GPT-4's tokenizer treats it as a single token. The numeric ID is some specific integer. The model never sees `s`, `t`, `r`, `a`, `w`, `b`, `e`, `r`, `r`, `y`. It sees that one integer.
...
This is also why the same model does fine on numerical reasoning that does not require it to look inside its own tokens. "What is 17 + 28?" is a problem whose ingredients (the numbers, the plus operator, the result) are all standard tokens. The model has the right pieces and the right operations. "How many Rs in strawberry?" requires looking at the *characters inside* a token. Different problem entirely.
...
- **`<|begin_of_text|>` and `<|end_of_text|>`** (often written `BOS` and `EOS`). These mark the start and end of a piece of text. The model has been trained to expect them at certain places.
- **Chat-role markers** like `<|im_start|>` and `<|im_end|>`, or `system` / `user` / `assistant`. In chat-tuned models these mark whose turn is whose. Without them, the model would not be able to tell where your message ends and its previous message begins.
- **`<pad>`**, **`<unk>`**, **`<mask>`**. Padding to make batches uniform; an unknown-token fallback (rarely needed in subword tokenizers); a mask token used during training for fill-in-the-blank objectives.
...
One thing worth flagging, though we will not go deep here: special tokens are also where prompt-injection attacks live. If a malicious input contains the literal string `<|im_start|>user` and the tokenizer parses it as the special token instead of as plain text, the attacker has just inserted a fake conversation turn into the model's input. This is the structural reason API providers strip or escape these markers in user input. The full story belongs in its own future lesson; for now, file it as "tokenization has a security surface that becomes important later."
