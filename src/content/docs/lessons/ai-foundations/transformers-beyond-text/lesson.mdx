...
That observation carried weight beyond image classification. Once ViT showed that the transformer block worked for non-text inputs, the field started applying transformers to other modalities. Multimodal models (vision-language, audio-text, etc.) typically use a ViT-style image encoder plus a text-decoder LLM, with cross-attention letting the text model see image tokens. The lecturer mentions LLaVA as one popular example.

The pattern is the same as any modern multimodal model you have used (GPT-4V, Claude with vision, Gemini, etc.): an image goes through a ViT or ViT-style encoder, comes out as a sequence of tokens, and gets concatenated with text tokens for the LLM to work with.
