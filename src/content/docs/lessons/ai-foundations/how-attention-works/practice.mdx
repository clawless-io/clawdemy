...
<Flashcard question="What is self-attention, in one sentence?">
...
<Flashcard question="What three vectors does every token get in self-attention, and where do they come from?">

Query (`Q`), Key (`K`), Value (`V`). Each comes from multiplying the token's embedding by a trained weight matrix (`W_Q`, `W_K`, `W_V`).

</Flashcard>

<Flashcard question="What does the dot product `Q · K^T` measure?">
...
<Flashcard question="Why is the dot product divided by `√d_k`?">
...
<Flashcard question="What does softmax do, in plain terms?">

It turns a list of arbitrary real numbers into a list of weights that sum to 1.0 and emphasize the largest values.

</Flashcard>

<Flashcard question="What is the output of a self-attention computation for one token?">
...
<Flashcard question="What is the only difference between self-attention and cross-attention?">

The source of `Q`, `K`, `V`. Self: all three from the same sequence. Cross: `Q` from one sequence, `K` and `V` from another.

</Flashcard>

<Flashcard question="What were the two structural problems with RNNs that transformers fixed?">
...
<Flashcard question="In the library analogy, what do `Q`, `K`, `V` map to?">
...
<Flashcard question="What does the full attention formula look like?">

`Attention(Q, K, V) = softmax( (Q · K^T) / √d_k ) · V`

</Flashcard>

<Flashcard question="Are attention weights a faithful explanation of why a model produced a given output?">
...
<Flashcard question='Does a transformer "remember" your previous conversation across separate calls?'>
