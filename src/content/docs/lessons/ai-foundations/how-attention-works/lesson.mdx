...
**Step 3a: softmax.** Exponentiate each scaled score, sum them, and divide each by the total. The arithmetic gives:
...
**Step 3b: weighted sum of values.** Multiply each value vector by its weight and add the three vectors together.
