...
Here is the version that fails: "escalate to an adjuster if you are not confident." It reads like a safety measure. It is actually a blank cheque, because it delegates the escalation decision to the exact component whose judgment you were worried about.

The version that works is explicit, rule-based, and full of examples. Meridian's escalation criteria look like this:
...
Every human touchpoint in this lesson assumed someone is there to touch. Lesson 7 removes that assumption: agents running inside CI/CD pipelines, headless, with nobody at the keyboard. The escalation criteria you learned to write here stop being one safeguard among several and become the only human presence in the system. That is where designing them well stops being good practice and starts being the whole game.
