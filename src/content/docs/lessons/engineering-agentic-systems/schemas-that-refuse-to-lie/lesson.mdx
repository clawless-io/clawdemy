...
<ReadAlongDim
  src="https://audio.clawdemy.org/lessons/schemas-that-refuse-to-lie-lesson.mp3"
  slug="schemas-that-refuse-to-lie"
  cacheVersion="20260707"
  approxMinutes={21}
/>
...
The tool choice parameter (tool_choice) controls whether the model must call a tool. Three of its modes form a ladder of guarantees; a fourth mode, none, forbids tool calls entirely and has no role in an extraction pipeline, so this lesson sets it aside.
...
- **The honesty pattern transfers to how you ask AI anything.** Give the model an explicit permitted way out ("if the document does not say, tell me it does not say") and answers get more trustworthy. That is required-plus-nullable, practiced in plain conversation, in [Clawless](https://clawless.ai) or anywhere else you work with a model.
