...
The print flag runs Claude Code non-interactively: one prompt in, the result printed, then exit. It reads standard input and writes standard output like any command-line tool. A pipeline needs it because an interactive session pauses for permissions and decisions, and in CI every pause is a deadlock: the job would hang waiting for an answer that will never come.
...
**5. Why must the reviewer be a separate invocation from the generator?**
...
Keep the sketch. In the capstone lesson you will design a full system, and an automated check like this one makes a strong candidate for one of its parts.
