...
| Subagent | Works one slice in an isolated context | Inherits nothing; the brief is its whole world |
| Seam | Where context, failures, and sources cross | Every seam must justify itself, then be engineered |

**Channel in:** the delegation prompt. **Channel out:** the final message. Nothing else crosses.
...
| Line | Meaning |
| --- | --- |
| A subagent inherits nothing; the brief is its world | Write for an empty desk |
