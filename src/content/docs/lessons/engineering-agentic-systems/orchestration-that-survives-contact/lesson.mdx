...
description: "One coordinator, many subagents, real failures. Learn to brief agents that inherit nothing, split work cleanly, and keep sources attached."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
## A subagent starts with an empty desk

Here is the fact everything else in this lesson leans on: a subagent inherits nothing automatically. In the Claude Agent SDK, each subagent runs in its own fresh conversation, with its own empty context window. The docs state the boundary exactly: "Intermediate tool calls and results stay inside the subagent; only its final message returns to the parent." The traffic in the other direction is just as narrow. The subagent never sees the coordinator's conversation, its history, or the results of anything the coordinator did before. The only channel in is the text of the delegation prompt itself.
...
Anthropic's post states the production reality in four words: "Agents are stateful and errors compound." A subagent that hits a dead end is not an exception; on a long run it is a certainty. What matters is what crosses the seam when it happens, and there are two ways to get it wrong that between them cover most beginner orchestrators.
...
- **The brief is a transferable skill.** Objective, output format, sources, boundaries: that is simply good delegation, and it works on subagents, contractors, and new teammates alike. If you can write a brief that survives an empty desk, you can delegate to anything.
...
- A subagent inherits nothing automatically. It starts with an empty desk, and the delegation brief is its entire world. Objective, output format, tools and sources, boundaries.
...
**A subagent inherits nothing; the brief is its world.**\
