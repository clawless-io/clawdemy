---
title: 'Lesson: Grouping without labels: k-means clustering'
description: "Every model so far needed labels. Clustering drops them: you have data and no answers, and you want to find the natural groups hiding in it. K-means is the workhorse. This lesson walks the assign-and-update loop step by step on a small dataset, shows how to choose the number of clusters, and is honest about when clustering is the right tool and when it will mislead you by inventing groups that are not there."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
