---
title: 'Lesson: Asking the right questions: decision trees'
description: "Logistic regression draws one straight boundary. A decision tree takes a completely different approach: it asks a sequence of yes/no questions, like a flowchart, funnelling each example down to a prediction. This lesson shows how to read a tree, how it is built by choosing the question that best separates the classes, why an unrestrained tree overfits, and why a single tree is powerful but unstable, which sets up random forests."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
