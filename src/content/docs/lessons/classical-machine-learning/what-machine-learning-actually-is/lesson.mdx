---
title: 'Lesson: What machine learning actually is'
description: "Machine learning flips traditional programming on its head: instead of writing the rules, you hand the machine examples and let it find the pattern. This lesson draws the line between rules and learning from data, splits the field into supervised and unsupervised learning, says when machine learning is the wrong tool, and lands the one rule that governs everything that follows: a model is only as good as it does on data it has never seen."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
