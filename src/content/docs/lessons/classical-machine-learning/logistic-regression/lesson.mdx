---
title: 'Lesson: From a line to a probability: logistic regression'
description: "Linear regression predicts a number, but many real questions are yes-or-no. Logistic regression answers them by taking the same weighted sum of inputs and squashing it through an S-shaped curve into a probability between 0 and 1. This lesson shows why a plain line fails for yes/no problems, how the sigmoid fixes it, where the decision boundary sits, and how the model is fit by the gradient descent you just learned."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
There is no tidy least-squares formula for the best logistic regression, the way there was for a straight line. So this is the first place we cash in the previous lesson: logistic regression is fit by **gradient descent**, searching for the coefficients that minimize a loss built for probabilities (it rewards confident-correct predictions and punishes confident-wrong ones especially hard). The procedure is exactly the downhill walk from lesson 3, applied to a different loss.
