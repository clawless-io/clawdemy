---
title: 'Lesson: From language models to large multimodal models'
description: "L1 said the most common way to build a multimodal model is to take an existing LLM and attach a vision encoder. This lesson walks that path concretely through CogVLM, the canonical example: how a pretrained vision transformer produces image tokens, how a bridge module projects them into the LLM's input space, where CogVLM goes deeper than the simplest LLaVA-style design with its 'visual expert' lanes, and how the model is trained in two stages without destroying the underlying language abilities."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
