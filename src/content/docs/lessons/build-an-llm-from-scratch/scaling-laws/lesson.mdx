...
Their finding upended the field: **model size and training data should scale roughly equally with compute.** Optimal model size `N*` and optimal data `D*` both scale as compute^0.5. Concretely, the **Chinchilla-optimal ratio** is about:
...
The corrected guidance: given a compute budget `C`, pick `(N, D)` so that `6ND = C` and `D / N ~= 20`. That single rule, paired with the lesson-3 architecture and lesson-2 accounting, gives you a defensible starting point for a training run.
...
- **Pick a compute-optimal `(N, D)`** for a target compute budget, as above.
...
- **Kaplan (2020)** suggested spending compute disproportionately on parameters; **Chinchilla (2022)** corrected this: optimal model size and data should scale **roughly equally** with compute. Rule of thumb: `D* ~= 20 * N*` tokens per parameter.
...
- **Scaling laws give predictive power:** fit curves at small/medium scales, extrapolate to the target. Use them to choose `(N, D)`, predict loss before committing, and sanity-check runs in flight.
...
Scaling laws turn "how big should I make it?" from folklore into arithmetic: `6ND = C` and `D / N ~= 20` is the Chinchilla starting point, adjusted upward in tokens when inference cost is a serious factor. The Phase 3 work that follows, evaluation, data, and post-training, refines what counts as "good" in those equations.
