...
- A **forward pass** costs about `2 * N` FLOPs per token (each parameter participates in roughly one multiply-add).
...
A surprising amount of model code is not math but **reshaping tensors**, and written with raw `.view`, `.permute`, and `.transpose` it becomes an unreadable, bug-prone puzzle of bare numbers. **einops** fixes this by letting you name the dimensions. Compare splitting a packed attention tensor into heads:
...
- **Compute is FLOPs, and a matrix multiply (m by k times k by n) costs about `2 * m * n * k`.** The model is mostly matmuls, so this is most of the counting.
- **The 6ND rule:** training costs about `6 * N * D` FLOPs (N parameters, D tokens), two for the forward pass and four for the backward. It scopes a training run from two numbers.
- **Training memory is roughly `16N` bytes in fp32:** parameters (4N) + gradients (4N) + Adam optimizer states (8N), before activations, which scale with batch and sequence length. Memory is often the binding constraint.
...
- **einops names tensor dimensions** (`rearrange(x, "b s (h d) -> b h s d", h=n_heads)`), making shape transformations readable and far less bug-prone than raw `.view`/`.permute`.
