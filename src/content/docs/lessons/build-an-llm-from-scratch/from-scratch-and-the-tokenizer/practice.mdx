...
- **Merge 1:** the pair `a a` is most frequent. Replace it with `Z`. Sequence becomes `Z a b d Z a b a c`.
- **Merge 2:** now `Z a` is most frequent (it appears twice). Replace with `Y`. Sequence becomes `Y b d Y b a c`.
- **Merge 3:** now `Y b` is most frequent (twice). Replace with `X`. Sequence becomes `X d X a c`.

The sequence went from 11 symbols to 5, and you learned three merge rules (`Z=aa`, `Y=Za`, `X=Yb`) plus the new vocabulary entries. That is BPE training in miniature: count pairs, merge the most frequent, repeat. Your "encode" of new text would apply these same merges in order.
