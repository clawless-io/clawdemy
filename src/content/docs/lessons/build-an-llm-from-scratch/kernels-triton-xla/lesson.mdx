...
Two threads tie together. First, FlashAttention is exactly the fusion that the **grouped-query attention** of lesson 4 also supports, by shrinking the KV cache so more of it fits in fast memory. The two combine for long-context inference. Second, fusion is precisely what raises **arithmetic intensity** from lesson 2 and 5: instead of `2 ops / 1 byte transferred`, you get `many ops / 1 byte transferred`, which is the literal definition of moving from memory-bound toward compute-bound. The hardware lesson said "stage data into fast memory and reuse it"; this lesson is how you actually do that in code.
