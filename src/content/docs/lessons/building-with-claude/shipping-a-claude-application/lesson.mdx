---
title: "Lesson: Shipping a Claude application"
description: "Lesson 12 of Track 22 (Building with Claude), the track closer. The five production disciplines that turn what you built in L1-L11 into a shipped application. (1) Cost monitoring via the Anthropic Usage and Cost Admin API (the /v1/organizations/usage_report/messages and /v1/organizations/cost_report endpoints, an Admin API key starting sk-ant-admin..., bucket_width 1m/1h/1d, filter and group_by model + workspace + api_key + service_tier; data appears within about 5 minutes) plus the per-call usage fields from L2 and L7 (input_tokens, output_tokens, cache_creation_input_tokens, cache_read_input_tokens, usage.iterations for compaction). (2) Latency budgets that the L2/L3/L7 levers (streaming, effort, caching, batching) move deliberately. (3) Eval-set discipline from L9's pattern 5 (cross-track to T21 L7 LLMOps). (4) Rollout discipline (feature flags, canary, A/B, rollback, request_id from L2 on every call). (5) Lifecycle and deprecation handling (Anthropic's at-least-60-days notice; Active → Legacy → Deprecated → Retired; date-pinning + Models overview as the canonical source). The rollout checklist that pulls L1-L11 together for a real production deploy."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
{/* COMPONENT PLACEHOLDER (Lead wires at promotion): <SourceLecture url="https://platform.claude.com/docs/en/manage-claude/usage-cost-api" title="Anthropic public Claude docs (Usage and Cost Admin API + Model deprecations + Pricing + Rate limits)" instructor="Anthropic" /> */}
