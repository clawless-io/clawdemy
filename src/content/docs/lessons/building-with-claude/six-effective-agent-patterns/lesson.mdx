---
title: "Lesson: Six effective-agent patterns"
description: "Lesson 9 of Track 22 (Building with Claude). The canonical patterns the L8 loop substrate takes. Five workflow patterns plus the open-ended agent itself: prompt chaining (decompose into a fixed sequence; trade latency for accuracy); routing (classify and direct to a specialized followup); parallelization (sectioning for independent subtasks, voting for multiple perspectives); orchestrator-workers (a central LLM dynamically breaks down tasks and delegates); evaluator-optimizer (one LLM generates while another evaluates in a loop); the autonomous agent (open-ended problems where steps cannot be hardcoded). For each: the verbatim Anthropic definition, when to use, the named examples in the source post, the sketch on the L8 loop, and the trade-off that decides whether the pattern fits."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
{/* COMPONENT PLACEHOLDER (Lead wires at promotion): <SourceLecture url="https://www.anthropic.com/engineering/building-effective-agents" title="Anthropic engineering, 'Building Effective AI Agents' (Erik S. and Barry Zhang, 2024-12-19)" instructor="Anthropic" /> */}
