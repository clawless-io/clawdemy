---
title: "Lesson: The Messages API in production"
description: "Lesson 2 of Track 22 (Building with Claude). The gap between a one-shot script (lesson 1) and code you can put behind real users. Streaming for interactive UIs and long generations, the error-code map (429 rate limits, 500 / 529 platform errors, 413 too-large) and how to classify each for retry, what the official SDKs do for you automatically, the request_id you must log from day one, and the Message Batches API (50 percent cheaper, async, finishes in under an hour) for high-volume non-interactive work."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
{/* COMPONENT PLACEHOLDER (Lead wires at promotion): <SourceLecture url="https://platform.claude.com/docs/en/build-with-claude/working-with-messages" title="Anthropic Academy 'Building with the Claude API' (structural mirror) + Anthropic public Messages API, Streaming, Errors, and Batches docs" instructor="Anthropic" /> */}
