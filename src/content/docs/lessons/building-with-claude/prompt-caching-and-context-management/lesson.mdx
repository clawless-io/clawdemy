---
title: "Lesson: Prompt caching and context management"
description: "Lesson 7 of Track 22 (Building with Claude). The cost-and-staleness lever for a tool-heavy application. Prompt caching (cache_control on the system prompt, tool definitions, and stable message content; 5-minute and 1-hour TTLs; the 1.25x and 2x write multipliers and the 0.1x cache-hit price that drops repeated-prefix costs by about 90 percent; the four-breakpoint maximum; the usage fields that tell you whether your prefix is actually hitting). Context windows (1M tokens for Opus and Sonnet 4.x, 200K elsewhere; context rot as a real ceiling). Server-side compaction (replaces older history with a summary at a configurable trigger; the recommended default for long-running sessions). Context editing (tool result clearing and thinking block clearing for surgical control). The unifying frame: cache the stable parts, compact or clear the long parts, and pick the lever that matches your session's shape."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
{/* COMPONENT PLACEHOLDER (Lead wires at promotion): <SourceLecture url="https://platform.claude.com/docs/en/build-with-claude/prompt-caching" title="Anthropic public Claude docs (Prompt caching + Context windows + Compaction + Context editing)" instructor="Anthropic" /> */}
...
- **Compaction is the default long-session lever.** Beta header *compact-2026-01-12*; edit *compact_20260112*; default trigger 150K input tokens. Replaces older history with a summary block; same model is used for summarization (extra sampling step billed in *usage.iterations*). Cache the system prompt separately so it survives.
- **Context editing is the surgical lever.** Beta header *context-management-2025-06-27*. Two strategies: *clear_tool_uses_20250919* (use *clear_at_least* to make cache invalidation pay back) and *clear_thinking_20251015* (the *keep* parameter; thinking-block clearing preserves more cache than tool-result clearing).
