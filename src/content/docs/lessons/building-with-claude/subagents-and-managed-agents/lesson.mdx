...
{/* COMPONENT PLACEHOLDER (Lead wires at promotion): <SourceLecture url="https://platform.claude.com/docs/en/managed-agents/overview" title="Anthropic public Claude docs (Subagents in the Agent SDK at code.claude.com + Claude Managed Agents overview at platform.claude.com)" instructor="Anthropic" /> */}
...
The *agent_toolset_20260401* tool type enables the full pre-built set (Bash, file operations, web search, web fetch, and more). The session goes idle on a *session.status_idle* event; you can resume by sending another user event.
