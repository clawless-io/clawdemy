---
title: "Lesson: Server-side tools and built-ins"
description: "Lesson 5 of Track 22 (Building with Claude). The other half of L4's client-vs-server distinction. Three categories of Anthropic-provided tools: server tools that Anthropic executes (web_search at $0.01 per search, code_execution that is free when used with web_search_20260209 or web_fetch_20260209, web_fetch), Anthropic-schema client tools you still execute but do not have to author the schema for (bash, computer use as a beta, memory, text_editor), and tool_search (the scale tool that dynamically loads tools from a catalog of up to ten thousand). The server_tool_use response shape that differs from L4's tool_use, the pause_turn stop reason for server-side multi-iteration loops, and the per-tool pricing layer that sits on top of standard tokens."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
{/* COMPONENT PLACEHOLDER (Lead wires at promotion): <SourceLecture url="https://platform.claude.com/docs/en/agents-and-tools/tool-use/server-tools" title="Anthropic Academy 'Building with the Claude API' (server-tools sections) + Anthropic public Web search, Code execution, Web fetch, Computer use, Tool search docs" instructor="Anthropic" /> */}
...
Two version strings to know. *web_search_20260209* is the current version with **dynamic filtering** (the model writes code to post-process results before they hit the context window, cutting token spend by often more than half). *web_search_20250305* is the previous version without dynamic filtering. Dynamic filtering uses code execution as the internal substrate; you do not need to declare *code_execution_20260120* separately to get it.
...
**Pricing is the key fact.** **Code execution is free when used with *web_search_20260209* or *web_fetch_20260209* in the same request.** When either of those is declared, code-execution tool calls do not carry the per-execution charge beyond standard input and output tokens. Standalone *code_execution_20260120* (without web_search or web_fetch in the request) carries the standard per-execution pricing layer.

There are three shapes to recognize:

**Search plus computation, one tool declared.** This is the canonical case from the docs. Declare only *web_search_20260209*; dynamic filtering runs code internally to filter search results and perform the computation:
...
**Search plus top-level code execution.** If you also want the model to call *code_execution* as a top-level tool for tasks unrelated to filtering search results (plotting, file generation, a calculation that does not flow from search), declare both. The per-execution charge stays waived because *web_search_20260209* is in the request:
...
**Computation only, no search.** Declare *code_execution_20260120* alone; standard per-execution pricing applies.
...
Two versions: *web_fetch_20260209* is the current version with dynamic filtering; *web_fetch_20250910* is the previous version without. The same pricing rule as web_search applies: code execution is free when *web_fetch_20260209* is in the request (dynamic filtering uses code execution internally; declare *code_execution_20260120* alongside only if your task also needs top-level code execution).
...
**Two variants:** *tool_search_tool_regex_20251119* (Python *re.search* patterns, max 200 characters) and *tool_search_tool_bm25_20251119* (natural-language queries). The regex form is precise; the BM25 form is forgiving.
...
- **Per-tool fee**: web_search at $0.01 per search; code_execution free when used with web_search_20260209 or web_fetch_20260209, standard per-execution pricing otherwise.
...
- **web_search:** $0.01 per search, real-time results with always-on citations, two versions (20260209 with dynamic filtering, 20250305 without). Optional *max_uses*, *allowed_domains*, *blocked_domains*, *user_location*.
- **code_execution is free when used with *web_search_20260209* or *web_fetch_20260209*.** Standalone *code_execution_20260120* otherwise. Not ZDR-eligible.
- **web_fetch** retrieves full content of a specific URL; two versions (*web_fetch_20260209* with dynamic filtering, *web_fetch_20250910* basic); same pricing rule for code_execution applies.
