...
**Causal language modeling** is the GPT objective: predict the next token given everything before it. Same self-supervised idea, different head (`AutoModelForCausalLM`) and the same collator with `mlm=False`. This is how you would pretrain a small model from scratch, or continue training one on your own text. Perplexity again.
...
Everything else, the `map`-to-tokenize, the `TrainingArguments` (here `Seq2SeqTrainingArguments`), the `compute_metrics` discipline, the push to the Hub, is the loop you already know.
...
- **One loop underlies every task:** load and clean data, tokenize (with any alignment), load `AutoModelFor<Task>`, pick a collator and metric, train, evaluate, push. Only the head, the label shape, and the metric change.
...
- **Masked and causal language modeling need no hand-made labels:** the text supervises itself via `DataCollatorForLanguageModeling` (set `mlm=False` for causal). Metric: perplexity.
