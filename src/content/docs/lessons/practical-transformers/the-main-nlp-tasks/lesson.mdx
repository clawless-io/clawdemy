...
3. Load a model with the right head (the matching task-specific model class).
...
**Summarization** and **translation** are both sequence-to-sequence: an input sequence in, a different sequence out. They use the sequence-to-sequence model (T5, BART) and share two wrinkles you have not met yet:
...
2. **Sequence-to-sequence needs its own tools.** Summarization and translation use the sequence-to-sequence Trainer, the sequence-to-sequence data collator, and generation-based metrics (ROUGE, BLEU), because the target is a sequence, not a label.
