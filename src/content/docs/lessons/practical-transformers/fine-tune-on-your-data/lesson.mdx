---
title: "Lesson: Fine-tune a pretrained model on your own data"
description: The hands-on heart of Phase 1. Take a pretrained model and continue training it on a task-specific dataset using the Trainer, then measure whether it actually got better. Data collator, TrainingArguments, Trainer, evaluation with compute_metrics.
sidebar:
  order: 2
  label: Lesson
artifact: lesson
...
Keep a notebook open, and this time you want a GPU. Training on a CPU works but is painfully slow; Google Colab gives you a free GPU, which is the path of least resistance. If you have not, `pip install transformers datasets evaluate`.
...
One piece here is new and worth a pause: the **data collator**. In the last lesson we padded every sentence to the same length up front. That wastes compute, because the longest example in the whole dataset forces every batch to be that long. `DataCollatorWithPadding` does **dynamic padding** instead: it pads each batch only to the length of the longest example *in that batch*, right as the batch is assembled. Shorter batches stay short. You hand it to the `Trainer` and forget about it.

## The model: a head swap you should expect

Load the model the way you did in lesson 2, but with one new argument: `num_labels`, because MRPC has two classes (paraphrase or not).
...
The `Trainer` takes the model, the arguments, the train and validation splits, the collator, and the tokenizer (passed as `processing_class`, which tells the Trainer how to process data). Then one method call starts training.
...
`trainer.train()` runs the fine-tuning (a couple of minutes on a GPU) and reports the training loss as it goes. When you pass a tokenizer as `processing_class`, the Trainer defaults its collator to `DataCollatorWithPadding` anyway, so the explicit `data_collator` line is optional; we keep it to make the moving part visible.

## Evaluation: did it actually get better?

Here is the catch: `trainer.train()` on its own tells you the training loss is going down, but not whether the model is any good on data it has not seen. Loss going down is necessary, not sufficient. To get a real answer you add two things: tell the Trainer to evaluate (an `eval_strategy`), and give it a function that computes a metric you understand.
...
Those `predictions.predictions` are logits again (every transformer outputs logits, as you saw in lesson 2): 408 examples, 2 labels each. To turn logits into a concrete prediction, take the index of the larger value in each row with `argmax`:
...
Now compare `preds` to the true labels using the `evaluate` library, which knows the standard metrics for named datasets:
...
Then rebuild the Trainer with `eval_strategy="epoch"` and the new function, so it reports real metrics at the end of every pass through the data. Use a fresh `TrainingArguments` and a fresh model, otherwise you are just continuing to train the model you already trained:
...
The `Trainer` packages modern training practices behind single arguments in `TrainingArguments`. Three you will reach for:

- `fp16=True` turns on mixed-precision training: faster, and uses less GPU memory, usually for free.
- `gradient_accumulation_steps=4` simulates a larger batch size when your GPU cannot fit one, by accumulating gradients across several small batches before stepping.
- `learning_rate=2e-5` and `lr_scheduler_type="cosine"` adjust the most important hyperparameter and how it decays over the run.
...
- **A data collator handles dynamic padding:** `DataCollatorWithPadding` pads each batch to its own longest example instead of padding the whole dataset to one length.
...
- **`Trainer` assembles model, args, datasets, collator, and tokenizer**, and `trainer.train()` runs the loop.
- **A falling training loss is not enough.** Set an `eval_strategy` and a `compute_metrics` function (logits to predictions with `argmax`, then score with `evaluate`) to measure quality on held-out data.
