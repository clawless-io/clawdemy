---
title: "Lesson: Debug your training and get unstuck"
description: The close of Phase 2, and the most universally useful lesson in the track. How to read a Python traceback, debug a pipeline systematically, build a minimal reproducible example, and ask the community for help in a way that actually gets answered.
sidebar:
  order: 2
  label: Lesson
artifact: lesson
...
The message says something does not have a `.size()`. Climbing the traceback, the failing library line is `input_shape = input_ids.size()`, and `.size()` is a method on tensors, not on Python lists. So `input_ids` is a list when it should be a tensor. Check the type to confirm (`type(inputs["input_ids"])` returns `list`), and the cause is clear: the tokenizer was called without `return_tensors="pt"`, so it returned plain lists instead of PyTorch tensors. Add it back and the model runs. This is the core debugging move: read the error, form a hypothesis about *what kind of thing* is wrong (here, a type mismatch), and check it directly rather than guessing.

A related tool: when a model will not load, inspect what is actually in the repository. `list_repo_files(repo_id)` from `huggingface_hub` shows the files, and a missing `config.json` explains an `OSError` immediately. Look at the actual state of things; do not assume.

## Search the error, you are rarely the first

When the message and the traceback do not crack it, copy the error text into a search engine or Stack Overflow. This feels like cheating; it is not, it is the job. Most errors you hit have been hit before, and someone has posted the cause and the fix. The `'list' object has no attribute 'size'` error above is a textbook case: searching it surfaces the exact "add `return_tensors='pt'`" answer. Pasting an error message into a search bar is one of the highest-value five-second moves in all of programming.
...
- **Data formatting and labels:** the wrong column names, labels as strings when the model wants integers, or a `num_labels` that does not match your data. A surprising share of training errors are really data errors.
- **Tokenization and tensors:** forgetting `return_tensors`, or inconsistent lengths that should have been handled by a data collator.
- **The collator:** using `DataCollatorWithPadding` when a task needs `DataCollatorForSeq2Seq` or `...ForTokenClassification`, so labels are not padded correctly.
...
- **Your environment:** library versions and platform (for Transformers, `transformers-cli env` prints exactly this).
