...
A related tool: when a model will not load, inspect what is actually in the repository. The list-repo-files helper from the Hugging Face Hub library shows the files, and a missing `config.json` file explains an *OSError* immediately. Look at the actual state of things; do not assume.
