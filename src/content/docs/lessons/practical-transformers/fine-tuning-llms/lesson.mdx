...
If the dataset has a `messages` field, the `SFTTrainer` applies the model's chat template automatically, so you do not have to format the conversations yourself. Notice how much carries over: `load_dataset`, an `AutoModelFor...` load, a config object, a trainer, `train()`. The skills from Phases 1 and 2 are exactly the foundation this stands on; SFT is the same loop pointed at a generative model and instruction data.
...
- **SFT data is instructions and conversations** (role-tagged `system`/`user`/`assistant` messages), and the model's **chat template** (via `tokenizer.apply_chat_template`) lays them out the way the model expects. The wrong template breaks behavior.
- **`SFTTrainer` from TRL is the `Trainer` you know, specialized for SFT.** Same loop (data, `SFTConfig`, trainer, `train()`); it auto-applies the chat template for `messages` datasets.
