...
Uploading writes to your account, so the library needs a token proving who you are and which namespaces you can write to. In a notebook you call `notebook_login()` from `huggingface_hub` (in a terminal, `huggingface-cli login`), paste a token from your account settings, and it is cached for later uploads.
...
notebook_login() from huggingface_hub in a notebook, or huggingface-cli login in a terminal. You paste a token from your account settings; it is cached for later uploads.
