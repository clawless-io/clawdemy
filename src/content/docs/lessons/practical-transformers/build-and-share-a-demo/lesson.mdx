...
Three arguments tell the whole story. The first is the Python function to run; the next two are the *types* of the input and output components (here, text boxes). Call the `launch()` method and Gradio renders the interface inline in a notebook, or serves it at a local URL (`http://localhost:7860`) from a script. That is a complete web app: a labeled text box, a submit button, and an output area, from five lines.
...
For a quick share, pass a `share=True` flag to the `launch()` call:
...
- **A `launch(share=True)` call gives a temporary public link** (about 72 hours) that runs on your machine, good for a quick share.
