...
{/* COMPONENT PLACEHOLDER (Lead wires at promotion): <SourceLecture url="https://davidstarsilver.wordpress.com/teaching/" title="David Silver, UCL Reinforcement Learning Course, Lecture 3: Planning by Dynamic Programming (value iteration)" instructor="David Silver" /> */}
...
Read it slowly. For each state s and each candidate action a, you compute the immediate reward R(s, a) plus the discounted expected value at the next state under the **current** value estimate V_k. Then you take the max over a and that is the new value at s. You do this for every state in one sweep, then repeat.

There is no policy in the loop. The implicit policy at iteration k is "act greedily with respect to V_k", but you do not need to write it down until you want to act. Compare with policy iteration, which maintained an explicit pi_k throughout and changed it on improvement steps. Value iteration just chases V^*; the policy is a byproduct.

When does V stop moving? Two practical answers:

- **Theoretical:** keep going until || V_(k+1) - V_k ||_infinity < some small epsilon (the largest entry-wise change is tiny).
...
so the value-iteration update is just V_(k+1) = T^* V_k. The fact that makes everything work is:

> **The Bellman optimality operator T^* is a contraction in the sup-norm with constant gamma.**
...
A single application of T^* shrinks the maximum entry-wise gap by a factor of gamma. Applied k times, it shrinks the gap by gamma^k. Because V^* is the unique fixed point of T^* (by the Bellman optimality equation), iterating from any starting V_0 gives V_k = (T^*)^k V_0, and the distance to V^* shrinks geometrically with rate gamma. Concretely, the error after k iterations is at most a constant times gamma^k.
...
## Worked example: value iteration on the H/S MDP
...
Initialize V_0(A) = V_0(B) = 0 and apply T^* sweep by sweep.
...
Greedy policy implied by V_1: at A pick stay (Q = 1 > 0); at B pick switch (Q = 2 > -1). That is already **pi^***, the same policy policy iteration converged to.

**Iteration 2.** Using V_1 = (1, 2):
...
**Iteration 4.** Using V_3 = (2.71, 3.71):
...
You can see the pattern: V is creeping toward the fixed point V^* = (10, 11) that policy iteration found exactly in two rounds. After four sweeps you are at (3.439, 4.439), still well short. The geometric convergence at rate gamma = 0.9 means errors shrink by a factor of about 10 every 22 iterations; full numerical convergence of V is slow.
...
- **Q-learning is value iteration on sampled transitions.** When you do not know P and R, you cannot compute the full expectation under P inside the max. So you replace it with a sample: from an observed (s, a, r, s') you bootstrap a target r + gamma * max_a' Q(s', a') and nudge Q(s, a) toward it. That is exactly value iteration's update with the expectation replaced by a sample (lesson 8 develops it in full).
- **DQN minimizes the Bellman optimality residual.** A deep Q-network approximates Q with a neural network and uses gradient descent to make Q(s, a) close to the Bellman optimality target r + gamma * max_a' Q(s', a'). The recursion is the same; the table is replaced by a function approximator. Lesson 9.
...
- **Maintaining the policy explicitly during VI.** There is no need to; the implicit policy is greedy with respect to V_k, and you can read it off at any time without writing it down.

## What you should remember

- **Value iteration** applies the Bellman optimality equation as a direct update: **V_(k+1)(s) = max_a [R(s, a) + gamma * sum_(s') P(s'|s, a) V_k(s')]**. Sweep all states, repeat until V (or the greedy policy) stops moving.
...
- The implicit policy at iteration k is **greedy with respect to V_k**, and it typically stabilizes well before V converges; using the policy as the stopping rule is a standard trick.
- Worked on the H/S MDP at gamma = 0.9 starting from V_0 = (0, 0): V_1 = (1, 2), V_2 = (1.9, 2.9), V_3 = (2.71, 3.71), V_4 = (3.439, 4.439), creeping toward V^* = (10, 11) found exactly by policy iteration. The greedy policy is **pi^* = (stay, switch) from iteration 1 on**.
