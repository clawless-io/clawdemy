...
- **TD bootstrap.** The target uses the parameterized Q function(s'), an estimate from the same network. If the network is wrong at s', the target is wrong, and the update at (s, a) follows a wrong signal.
