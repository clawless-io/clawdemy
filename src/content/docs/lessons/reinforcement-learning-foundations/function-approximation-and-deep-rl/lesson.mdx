...
{/* COMPONENT PLACEHOLDER (Lead wires at promotion): <SourceLecture url="https://davidstarsilver.wordpress.com/teaching/" title="David Silver, UCL Reinforcement Learning Course, Lecture 6: Value Function Approximation" instructor="David Silver" /> */}
...
Replace Q(s, a) (a number per (s, a) entry) with **Q_theta(s, a)** (a function of (s, a) and parameters theta). In the linear case:

```
Q_theta(s, a)  =  theta . phi(s, a)         (dot product with a feature vector phi)
```

where phi(s, a) is a hand-designed feature vector for the state-action pair. In the neural-network case, Q_theta is the output of a neural network with parameters theta (weights and biases) given (s, a), or given s with one output per action. The point is the same: a small number of parameters theta represents Q across a (potentially infinite) state space, and updating theta updates Q **everywhere at once**, not just at a single (s, a).

The objective is to make Q_theta(s, a) close to a target estimating Q^pi or Q^*. For Q-learning the target is the same one as the previous lesson:
...
(One action only, so dropping `a` for brevity.) Suppose theta_0 = 0, theta_1 = 0 initially. Observe a transition: x_t = 2, reward r = 1, next state x_(t+1) = 3, gamma = 0.9, step size eta = 0.1.
...
- **TD bootstrap.** The target uses Q_theta(s'), an estimate from the same network. If the network is wrong at s', the target is wrong, and the update at (s, a) follows a wrong signal.
- **Off-policy.** Q-learning's target uses the max over actions, not the action the agent actually took. So the network is being asked to fit a function (Q^*) that does not match the distribution of (s, a) it actually sees, the data distribution and the target distribution disagree.
...
- **Function approximation introduces a bias-variance trade of its own.** A small network may underfit (high bias, can't represent Q^*); a large one may overfit and amplify the triad's instability (high variance, brittle training). Tuning capacity is a design knob practitioners spend real time on.
...
- **Believing the Bellman recursion changes with function approximation.** It does not. The same r + gamma * max_{a'} Q(s', a') target sits inside, just computed by a learned function instead of a table lookup.
...
- **Tabular methods do not scale.** Beyond a few thousand discrete states, you need a **function approximator** (linear features or a neural network) parameterized by theta: **Q_theta(s, a)**.
- The objective is the **squared TD error** against the Bellman target: Loss(theta) = E[ (target - Q_theta(s, a))^2 ], with target = r + gamma * max_{a'} Q_theta(s', a') for Q-learning.
- The **semi-gradient update** is **theta <- theta + eta * delta * grad_theta Q_theta(s, a)**, where delta is the TD error and the target is treated as fixed when computing the gradient.
