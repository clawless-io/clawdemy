...
description: "Lesson 3 of Track 17 and the close of Phase 1. With the MDP nailed down, you need a way to say how good things are. The state-value V and action-value Q answer that, the expected total reward from a state or a state-action pair under a policy. Their defining property is recursive: value here equals one step of reward plus the discounted value at the next state. That recursion, in two forms (expectation under a policy, and optimality over the best action), is the Bellman equation, the mathematical heart of reinforcement learning."
