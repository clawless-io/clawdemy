...
- The bootstrap uses **max over a'** of Q(s_(t+1), a'), not Q at the action you actually take next. That is the Bellman optimality target, sampled.
...
A short on-policy alternative for contrast: **SARSA** has the same shape as Q-learning but uses the agent's actual next action a_(t+1) in the target:
...
## Worked example: five Q-learning steps on the H/S MDP
...
That is **pi^*** already, after only five Q-learning steps. The Q **values** are nowhere near their true Q^* of (10, 9.9, 8.9, 11), they will need many more updates to numerically converge, but the **policy ranking** (which action is best at each state) is correct very early. This is the same early-stabilization pattern from value iteration (lesson 5), now in the sample-based setting. In practice you keep iterating until Q converges, but for control purposes the greedy policy can be useful long before then.

Two things to notice about the off-policy max in action: at step 3, the target used `max(0.5, 0) = 0.5`, the **stay** value at A; the agent did not have to take stay at A right then for the target to use it. And at step 5, the target used `max(0, 1.225) = 1.225`, the value Q-learning **already knew** for B-switch from step 3, even though the agent's behavior step 5 was actually B-stay. That is the off-policy property doing work: the agent acts however, the target reasons about the best.
...
- **Confusing the Q-learning target with the SARSA target.** Q-learning uses `max_{a'} Q(s_(t+1), a')` (off-policy); SARSA uses `Q(s_(t+1), a_(t+1))` with the actual next action (on-policy). Same shape, different action choice in the target, different convergence behavior.
- **Skipping exploration.** Without epsilon-greedy (or another exploration scheme), many (s, a) pairs are never visited and Q-learning cannot converge to Q^*. Convergence proofs explicitly require infinite-visit conditions.
...
- The **Q-learning update** combines TD's sample bootstrap with the Bellman optimality max: **Q(s_t, a_t) <- Q(s_t, a_t) + alpha * [ r_(t+1) + gamma * max_{a'} Q(s_(t+1), a') - Q(s_t, a_t) ]**.
...
- On the H/S MDP at alpha = 0.5, gamma = 0.9, five hand-traced Q-learning steps from Q = 0 give Q = (0.975, 0, 0.05125, 1.225); the greedy policy is already **pi^* = (A: stay, B: switch)** even though the Q values have not converged to Q^* = (10, 9.9, 8.9, 11).
