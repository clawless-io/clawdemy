...
description: "The first lesson sketched the agent-environment loop informally. To do anything precise with it, algorithms, convergence arguments, computable updates, you need a formal object, and that object is the Markov Decision Process. This lesson lays out the MDP tuple (states, actions, transition probabilities, reward function, discount), explains the Markov property and why it matters, defines a trajectory and the discounted return, walks the return on a small two-state example at three discount values to make the role of gamma concrete, and connects all of it to how a real system has to engineer its state to keep the Markov property intact."
...
{/* COMPONENT PLACEHOLDER (Lead wires at promotion): <SourceLecture url="https://davidstarsilver.wordpress.com/teaching/" title="David Silver, UCL Reinforcement Learning Course, Lecture 2: Markov Decision Processes" instructor="David Silver" /> */}
...
Each step is one execution of the loop from lesson 1. The convention used throughout this track is that the agent's action **a_t** is taken in state **s_t**, and the resulting reward **r_(t+1)** and next state **s_(t+1)** arrive on the next time step. Watch that subscript, off-by-one errors are the most common bookkeeping bug in RL.

## The return and the discount factor

The agent's goal is total reward, not per-step reward. The **return** from time t, written G_t, is the total reward accumulated from time t onward. There are two cases.
...
- **Off-by-one on reward indexing.** With the convention used here, a_t is taken in s_t and the resulting r_(t+1) and s_(t+1) arrive on the next step. The first reward in a trajectory is r_1, not r_0. Mixing conventions is one of the most common bookkeeping errors in RL.
...
- A **trajectory** under a policy is a sequence s_0, a_0, r_1, s_1, a_1, r_2, ... The **return** is **G_t = r_(t+1) + gamma * r_(t+2) + gamma^2 * r_(t+3) + ...**, the discounted total reward from t on.
