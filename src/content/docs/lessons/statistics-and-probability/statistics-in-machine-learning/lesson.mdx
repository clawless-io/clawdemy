...
description: "The capstone of Track 9. It walks every tool from the track into a real machine-learning workflow: describing data before modeling, reading model outputs as conditional probabilities, expected value as the training objective, and the big one, evaluation as inference, where a test set is a sample, a metric is an estimate with a confidence interval, and comparing models is a hypothesis test. It draws a clean boundary to the Classical ML track for the model-scoring toolkit, and closes on the track's through-line: statistics is the discipline of not fooling yourself about uncertainty."
