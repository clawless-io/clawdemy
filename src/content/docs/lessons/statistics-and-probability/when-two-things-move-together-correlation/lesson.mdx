...
The scatterplot shows the relationship; the **correlation coefficient** puts a single number on it. Written as *r*, it always lands between **-1 and +1**, and it carries two pieces of information at once.

- **The sign is the direction.** Positive *r* means the two rise together; negative *r* means one rises as the other falls.
- **The magnitude is the strength.** An *r* near +1 or -1 means the points sit almost perfectly on a straight line. An *r* near 0 means no straight-line relationship: the cloud has no consistent tilt.

So *r* = 0.95 is a strong positive relationship (tight upward line), *r* = -0.9 is a strong negative one, and *r* = 0.1 is a weak relationship that is barely there.

Where does this number come from? It connects directly to the standardizing you met earlier in the track. Recall the z-score: how many standard deviations a value sits above or below its mean. The correlation coefficient is, in essence, the **average of the products of the two z-scores**, one for each variable, across all the points. When a point is above average on both variables (two positive z-scores) or below average on both (two negative z-scores), the product is positive and pushes *r* up. When a point is high on one and low on the other, the product is negative and pulls *r* down. Add those products up and average them, and you get a number that is positive when the variables move together and negative when they move oppositely. You will not compute *r* by hand here; the point is that it is built from the same standardized distances you already know.

## The catch inside the number: only straight lines

There is one technical limit worth burning in: the correlation coefficient measures only **linear** (straight-line) association. A relationship can be strong, obvious, and important, yet have an *r* near zero, simply because it is not a straight line.
...
- **Treating *r* near zero as "no relationship."** It means no *linear* relationship. A strong curve (the U-shape) can sit at *r* near zero, which is why you always look at the scatterplot.
- **Forgetting outliers move *r*.** A single extreme point can inflate or deflate the coefficient dramatically; the scatterplot shows it, the number alone does not.
...
- The **correlation coefficient** *r* lands between -1 and +1: the **sign** is the direction (together or opposite), the **magnitude** is the strength (near the ends means a tight straight line, near zero means no linear drift). It is built from the same standardized z-scores you met earlier.
- *r* measures **linear** association only, so a strong curved relationship can have *r* near zero. Look at the picture, not just the number.
