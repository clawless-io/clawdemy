...
description: "Numbers like the mean and standard deviation summarize data, but a picture shows you its shape, and shape carries information no single number can. This lesson builds the histogram from scratch, names the shapes you will meet (symmetric, right- and left-skewed, uniform, bimodal, and the bell curve), reconnects skew to the mean-versus-median gap from the previous lesson, and shows why looking at a feature's distribution before modeling catches problems (skew, outliers, two populations hiding in one column, class imbalance) that summary numbers quietly miss."
