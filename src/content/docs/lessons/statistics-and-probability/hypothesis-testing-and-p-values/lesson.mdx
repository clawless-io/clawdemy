...
- **Reading the p-value as the probability the null is true.** It is `P(data | null)`, not `P(null | data)`, the flipped conditional. A small p is evidence against the null, not its probability.
...
- The **p-value** is `P(data at least this extreme, given the null is true)`. A small p means the data is surprising under the null, which is evidence against it; below the threshold **alpha** (often 0.05) you call it **statistically significant**.
