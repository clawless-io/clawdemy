...
A different shape where the user's computer talks directly to the AI provider over an API, with no consumer-facing CDN or vendor middleware in between. The worked example is Clawless Computer, the desktop app this site's sister project ships: messages go from the user's computer to the AI provider, and the reply comes back, with no Clawless Computer server in between holding logs of the requests. The point is not that one architecture is better; it is that the architecture decides which parties along the path see what.
