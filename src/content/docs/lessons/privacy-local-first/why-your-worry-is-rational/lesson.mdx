---
title: "Lesson: Why your worry is rational: three things to actually worry about"
description: "Sarah's privacy anxiety about AI tools has specific, nameable causes, distinct from generic doom. This lesson names three of them in plain language: what gets seen along the way, what gets kept and whether it can leak, and what the vendor will or will not tell you. By the end you can name three concerns from your own situation and separate them from the headline-noise version."
sidebar:
  order: 2
  label: Lesson
artifact: lesson
...
When Aisha types a student's name into the new tool, the text does not stay on her laptop. It travels. It goes through her school's network, out to her internet provider, across the public internet, into the AI company's data center, through several handoffs at the AI company, into a model that is running on a server somewhere, back through the same chain, onto her screen. At several points along that path, the message is being handled by software written by people whose job is to keep that software running. Some of them can read it if they choose to and have a reason to. Some logging systems record bits of it automatically.

This is not a conspiracy theory. It is how the system works. The Electronic Frontier Foundation, a long-running digital rights group, makes the point clearly: tens of millions of people use chatbots to brainstorm, test ideas, and explore questions they might never post publicly or even admit to another person, and *these conversations reveal people's most sensitive information.* They give examples that translate into everyday situations: someone asking how to get abortion pills, how to protect themselves at a protest, how to escape an abusive relationship. A single chat thread can expose the kind of intimate detail once locked away in a handwritten diary.

Aisha's case is less intense but the same shape. A student's name plus a grade plus a comment about that student's behavior is, in aggregate, a small intimate fact about a child. It does not stop being intimate because the system processing it is a chatbot.
...
**Treating "privacy mode" as a privacy guarantee.** Many tools now have a "temporary chat" or "incognito" mode. Read what each one actually promises. Mozilla's research finds that, for example, one major chatbot provider's Temporary Chat says it will not use the conversation for training, will not show it in your history, and will store it for up to 30 days, which is a *real* protection but is narrower than the word "incognito" suggests. The thirty-day storage is real. The protection is real. They are not the same thing. Read the specifics.
