---
title: "Lesson: The chain rule, multiplying rates through a composition"
description: "The product rule handled functions multiplied together. The chain rule handles functions nested one inside another, and it says something simple: rates multiply through a composition. This is the single most-used calculus rule in machine learning, because backpropagation is exactly this rule applied through a network's layers."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
{/* DIAGRAM PLACEHOLDER: a pipeline x -> g -> u -> f -> output, with the rate g'(x) labeled on the first arrow and f'(u) on the second, and the total rate f'(g(x))·g'(x) shown as their product */}
