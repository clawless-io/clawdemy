...
{/* DIAGRAM PLACEHOLDER: a rectangle with width f(x) and height g(x), area labeled f(x)·g(x) */}
...
{/* DIAGRAM PLACEHOLDER: the rectangle with a top strip (f wide, g'·dx tall), a side strip (f'·dx wide, g tall), and a small corner block (f'·dx by g'·dx) shaded to show the three pieces of added area */}
