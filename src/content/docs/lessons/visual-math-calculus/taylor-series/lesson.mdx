...
{/* DIAGRAM PLACEHOLDER: a curve f(x) with its Taylor approximations at a point a overlaid: the tangent line (1st order), then a parabola (2nd order), then a higher-degree curve, each wrapping more tightly around f near a */}
