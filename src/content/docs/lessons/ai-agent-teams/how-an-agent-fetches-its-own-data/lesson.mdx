...
A **tool** changes that. A tool is a function the model is allowed to call: fetch a stock's price history, pull recent news, read a balance sheet. The model does not run the function itself; it asks for it ("call get_stock_data for this company"), the system runs the function, and the result is handed back to the model. Now the model can act in the world, not just describe it. This is the idea the foundational track called the thing that turns a model into an agent.
...
Everything below is quoted from that frozen snapshot (the short code `7e9e7b8` marks the exact version, so what you read here matches what you see on screen). The file is `tradingagents/agents/analysts/market_analyst.py`. You do not need to read every line; we pull out the parts that matter.
...
`get_stock_data` pulls the price history; `get_indicators` computes technical measures from it. That is the analyst's whole toolbox. A narrow set of tools for a narrow job, exactly the focus idea from lesson 1.
...
`bind_tools(tools)` is the line that matters: it tells the model "these are the functions you may call." From now on, when the model responds, it can either write text or ask to call one of its tools.
...
Read it in plain English: if the analyst's last message contains tool calls, send it to `tools_market` (the step that actually runs the requested tool). Otherwise, the analyst is done; move on.

And `tools_market` loops straight back to the analyst:
...
When the model stops asking for tools and just writes prose, that prose *is* the report. "No tool call" is the agent's way of saying "I have enough." The report it produces is saved into the team's shared state as `market_report`, the one durable thing this analyst leaves behind.
