...
The scalar lambda is the **eigenvalue**: the factor *v* gets scaled by. The equation says something strong. On the left, *M* does its full transformation to *v*. On the right, all that happened was multiplication by a single number. For an eigenvector, the entire matrix acts like a plain scalar. No rotation, no shear, just scaling.
...
Now the eigenvector for each. For lambda equals 3, form *M* minus 3*I*, the matrix with rows 0, 1 and 0, negative-1, and solve for the vectors it sends to zero. Both rows say y equals 0, with x free, so the eigenvectors are everything along the vector 1, 0. For lambda equals 2, form *M* minus 2*I*, the matrix with rows 1, 1 and 0, 0, and solve: the top row says x plus y equals 0, so y equals negative x, giving the line through 1, negative-1.
...
For lambda equals 3, *M* minus 3*I* has rows negative-1, 1 and 1, negative-1, and the equation "negative x plus y equals 0" gives y equals x, the line through 1, 1. For lambda equals 1, *M* minus *I* has rows 1, 1 and 1, 1, and "x plus y equals 0" gives the line through 1, negative-1. Check:
...
**Mixing up eigenvalue and eigenvector.** The eigenvalue is the number (the scaling factor); the eigenvector is the direction that gets scaled. In the equation "*M* times *v* equals lambda times *v*," *v* is the vector and lambda is the number.

## What you should remember

- **An eigenvector is a nonzero vector the transformation only scales, not rotates: *M* times *v* equals lambda times *v*.** The eigenvalue lambda is the scaling factor. Geometrically, eigenvectors are the directions that stay on their own line through the transformation; the eigenvalue is how much they stretch (or flip, if negative, or collapse, if zero).
