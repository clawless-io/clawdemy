...
For a 2x2 matrix, the determinant has a short formula, a-d minus b-c:
...
This is the signed area of the parallelogram spanned by the columns a, c and b, d. The quick way to see where a-d minus b-c comes from: enclose the parallelogram in a bounding rectangle of width a plus b and height c plus d, then subtract the rectangles and triangles in the corners that are not part of the parallelogram. When you tally the trimmed pieces, the leftover area works out to a-d minus b-c. You do not need to memorize the derivation, only the result: multiply the main diagonal, subtract the off-diagonal product.
...
**A pure stretch.** Take *M* the matrix with columns 3, 0 and 0, 2, which sends *i-hat* to 3, 0 and *j-hat* to 0, 2. The formula gives:
...
**A rotation.** Take the quarter-turn rotation *R* with columns 0, 1 and negative-1, 0. The formula gives:
...
**A collapse.** Take *M* with columns 2, 1 and 4, 2. Notice that the second column is 2 times the first: the columns are linearly dependent, lying on the same line. The formula gives:
...
**A reflection.** Take *M* with columns 1, 0 and 0, negative-1, which leaves *i-hat* alone but sends *j-hat* to 0, negative-1, flipping the plane across the x-axis. The formula gives:
...
**Mixing up the diagonals.** The formula is main diagonal minus off-diagonal: a-d minus b-c, not a-b minus c-d, nor a-c minus b-d. Multiply *a* by *d*, subtract *b* times *c*.
...
- **For a 2x2 matrix with rows a, b and c, d, the determinant is a-d minus b-c,** signed: the magnitude is the size scaling, and a negative sign means orientation flipped. A determinant of zero means the columns are dependent and space collapsed.
