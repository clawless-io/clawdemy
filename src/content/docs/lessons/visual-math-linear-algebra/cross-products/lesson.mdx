...
For two vectors *v* the pair a, b and *w* the pair c, d, the **2D cross product** is
...
The two vectors *v* and *w*, drawn from the origin, span a parallelogram. The **magnitude** of the cross product, the magnitude of *v* cross *w*, is the area of that parallelogram. The **sign** records orientation: which way you turn to get from *v* to *w*.

- **Positive *v* cross *w*:** *w* lies counterclockwise from *v*. Rotating *v* toward *w* through the smaller angle, you turn counterclockwise.
- **Negative *v* cross *w*:** *w* lies clockwise from *v*. The turn goes the other way.
- **Zero *v* cross *w*:** *v* and *w* are collinear, pointing along the same line (same direction or opposite). The parallelogram has flattened to a line segment, and its area is zero.
...
Here is the promised payoff. The 2D cross product *v* cross *w* is exactly the determinant of the 2x2 matrix whose columns are *v* and *w*:
...
The dot product was commutative: *v* dot *w* equals *w* dot *v*, order did not matter. The cross product is the opposite, **anti-commutative**:
...
**Confusing it with the dot product.** The dot product measures alignment and is symmetric (*v* dot *w* equals *w* dot *v*). The cross product measures spanned area and is antisymmetric (*v* cross *w* equals negative *w* cross *v*). Same two inputs, opposite character.
...
**Forgetting which diagonal.** The formula is a-d minus b-c, the same main-diagonal-minus-off-diagonal pattern as the determinant. It is not a-c minus b-d, nor a-b minus c-d.
...
- **The 2D cross product, *v* cross *w* equals a-d minus b-c, is the signed area of the parallelogram that *v* and *w* span.** The magnitude is the area; the sign is orientation: positive if *w* is counterclockwise from *v*, negative if clockwise, zero if they are collinear.
...
- **It is anti-commutative: *v* cross *w* equals negative *w* cross *v*,** the mirror image of the dot product's symmetry. Dot is symmetric and reports alignment; cross is antisymmetric and reports signed area.
