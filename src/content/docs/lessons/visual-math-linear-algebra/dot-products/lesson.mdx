...
where the magnitudes of *v* and *w* are the lengths of the two vectors and theta is the angle between them. This looks nothing like "multiply components and add," yet it gives the identical answer every time. That is the surprise the lesson is built around.
...
The geometric formula has a vivid reading when one of the vectors has length 1. Let *u-hat* be a **unit vector** (length exactly 1). Then *v* dot *u-hat* equals the length of *v* times the cosine of theta, and that quantity is the **signed length of the projection** of *v* onto the line through *u-hat*.

Picture dropping a perpendicular from the tip of *v* straight down onto the line that *u-hat* lies along. The distance from the origin to the point where it lands, the foot of the perpendicular, is the projection length. It is signed: positive if *v* leans the same way as *u-hat*, negative if it leans the opposite way. So *v* dot *u-hat* answers "how far does *v* reach along the *u-hat* direction?"
...
That projection picture is also where the full geometric formula comes from. With a unit vector, *v* dot *u-hat* is just the projection length. A general vector *w* is its length times its direction, so dotting against it scales the projection by that length:
...
The length of *w* is simply how much longer *w* is than a unit vector, stretching the projection by the same factor. That is why the lengths-and-cosine formula has both magnitudes in it.
...
**Pinning down a cosine.** Take the vector 3, 4 and the unit vector 1, 0 again. The algebraic formula gave 3. The geometric formula says the length of *v* times the length of *w* times the cosine of theta, and here the length of *v* is 5 (the 3-4-5 right triangle from the first lesson) and the length of *w* is 1, so:
...
The dot product is **commutative**: *v* dot *w* equals *w* dot *v*. From the algebraic formula this is obvious, since "v-1 times w-1 plus v-2 times w-2" is the same sum as "w-1 times v-1 plus w-2 times v-2." Geometrically it is less obvious (why should projecting *v* onto *w* give the same number as projecting *w* onto *v*?), and the duality insight is exactly what reconciles it: both are the single symmetric quantity, the length of *v* times the length of *w* times the cosine of theta, and the angle between two vectors does not care which one you name first.
...
**Forgetting the unit-vector condition for projection.** The quantity *v* dot *u-hat* is the projection length only when *u-hat* has length 1. If the second vector is not a unit vector, the dot product is the projection length scaled by that vector's length.

## What you should remember

- **The dot product turns two vectors into one number, two ways that always agree:** algebraically, multiply matching components and add ("v-1 times w-1 plus v-2 times w-2" and so on); geometrically, the length of *v* times the length of *w* times the cosine of theta. The sign tells you whether the vectors broadly agree (positive), are perpendicular (zero), or oppose (negative).
- **Dotting with a unit vector projects.** The quantity *v* dot *u-hat* is the signed length of *v*'s shadow on the *u-hat* direction: how far *v* reaches that way.
