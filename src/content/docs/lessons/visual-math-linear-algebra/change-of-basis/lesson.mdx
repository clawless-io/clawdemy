...
Suppose another observer, call her Jennifer, uses basis vectors `b1` and `b2`. We can write her basis vectors in our standard coordinates: say `b1 = [2, 1]` and `b2 = [-1, 1]`. Build the matrix whose columns are her basis vectors expressed in our system:
...
This `M` is the translator from Jennifer's language to ours. If Jennifer says a vector has coordinates `[x', y']` in her basis, she means "`x'` of `b1` plus `y'` of `b2`." In our coordinates, that is exactly `x' · b1 + y' · b2`, which is `M · [x', y']`. The columns of `M` are where her basis vectors land in our world, the same column trick from the transformations lesson, now read as a translation between languages.

**Worked translation.** Jennifer points to a vector and calls it `[1, 1]` in her basis. What do we call it? Apply `M`: `M · [1, 1] = 1 · [2, 1] + 1 · [-1, 1] = [1, 2]`. Her `[1, 1]` is our `[1, 2]`. Same arrow, two names.

## Translating from ours to hers

To go the other way, from our coordinates into Jennifer's, we undo `M`, which means applying `M^-1`. (This is where the inverses lesson pays off: the translation is reversible exactly because `M` is invertible, `det(M) ≠ 0`. If Jennifer's "basis" vectors were linearly dependent, they would not span the plane, `M` would collapse, and there would be no clean way back.)

For a 2x2 matrix there is a quick formula for the inverse: for `M = [[a, b], [c, d]]`,
...
Here `det(M) = (2)(1) - (-1)(1) = 3`, so `M^-1 = (1/3) · [[1, 1], [-1, 2]]`.

**Worked translation back.** We have a vector we call `[3, 2]`. What does Jennifer call it? Apply `M^-1`: `M^-1 · [3, 2] = (1/3) · [3 + 2, -3 + 4] = (1/3) · [5, 1] = [5/3, 1/3]`. Our `[3, 2]` is her `[5/3, 1/3]`.

Check the round trip, to confirm the two translations really are inverses. Take her `[5/3, 1/3]` back to our basis with `M`: `(5/3) · [2, 1] + (1/3) · [-1, 1] = [10/3, 5/3] + [-1/3, 1/3] = [9/3, 6/3] = [3, 2]`. We are back where we started. `M` and `M^-1` translate in opposite directions and cancel.
...
The rightmost `M` translates from her basis to ours, the middle `A` does the transformation in our basis, and `M^-1` translates back to hers. The sandwich is the transformation as Jennifer experiences it.

**Worked transformation translation.** Take the 90-degree counterclockwise rotation you have used since the transformations lesson, `R = [[0, -1], [1, 0]]`, and find its matrix in Jennifer's basis. Compute `M^-1 · R · M`.

First `R · M`, applying `R` to each column of `M`:
...
so `R · M = [[-1, -1], [2, -1]]`. Then apply `M^-1` to each of those columns, with `M^-1 = (1/3) · [[1, 1], [-1, 2]]`:
...
That matrix is far uglier than `[[0, -1], [1, 0]]`, but it describes the identical physical rotation. Jennifer's basis just happens to be an awkward one for talking about this particular spin. The operation did not change; only the language did.

To see why the sandwich is the right recipe, follow one of Jennifer's vectors through it one stage at a time. Take her `[1, 1]`. Translate it to our basis: `M · [1, 1] = [1, 2]`. Rotate it in our basis, where we know how: `R · [1, 2] = 1 · [0, 1] + 2 · [-1, 0] = [-2, 1]`. Translate the result back to her basis: `M^-1 · [-2, 1] = (1/3) · [-2 + 1, 2 + 2] = [-1/3, 4/3]`. Now apply the combined matrix `R_jennifer` directly to her `[1, 1]`: `[[1/3, -2/3], [5/3, -1/3]] · [1, 1] = [1/3 - 2/3, 5/3 - 1/3] = [-1/3, 4/3]`. Same answer. The three-step journey and the single sandwich matrix agree, because the sandwich is precisely those three steps bundled into one matrix.
...
**Confusing which matrix translates which way.** `M` (columns are the other basis written in our coordinates) translates *from* the other basis *to* ours. `M^-1` goes the other way. If you mix them up, you translate in the wrong direction.

**Forgetting the sandwich order.** `A_jennifer = M^-1 · A · M`, read right to left: into our basis, transform, back to hers. Getting the order or the inverses wrong gives a matrix that describes a different operation.
...
- **To translate coordinates, use the basis matrix `M`** whose columns are the other basis written in your coordinates: `M` takes their coordinates to yours, and `M^-1` takes yours to theirs (which requires `det(M) ≠ 0`).
- **A transformation gets a different matrix in a different basis, given by the sandwich `M^-1 · A · M`.** Same physical operation, different numerical description. The right basis can make a transformation's matrix dramatically simpler, which is exactly what the next lesson chases with eigenvectors.
