---
title: "Lesson: Stepping up to 3D"
description: "Everything so far lived in the flat plane. This lesson steps off the page into three dimensions and shows that almost nothing changes: a third basis vector, a third column, one more number per vector, and every rule you already know carries straight over."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
`i-hat` points one unit along x, `j-hat` one unit along y, and the new arrival `k-hat` one unit along z, straight out of the old flat plane toward you. Every 3D vector is a combination of these three: `[3, 4, 5]` is just `3 · i-hat + 4 · j-hat + 5 · k-hat`, three steps along x, four along y, five along z.
...
The matrix-vector product extends in the obvious way. For `v = [x, y, z]`,
...
{/* DIAGRAM PLACEHOLDER: the three basis vectors i-hat, j-hat, k-hat along the x, y, z axes, and an arbitrary vector [3,4,5] reached by stepping along each */}
...
Looking down the z-axis, this is just the familiar 2D rotation happening in the x-y plane, while z is left alone. So `i-hat` swings to `[0, 1, 0]`, `j-hat` swings to `[-1, 0, 0]`, and `k-hat` stays exactly where it is at `[0, 0, 1]`, because points on the z-axis do not move when you spin around it. The matrix:
...
Apply it to `[3, 4, 5]`: `3 · [0, 1, 0] + 4 · [-1, 0, 0] + 5 · [0, 0, 1] = [0, 3, 0] + [-4, 0, 0] + [0, 0, 5] = [-4, 3, 5]`. The x and y parts rotated just like the old 2D example (`[3, 4]` became `[-4, 3]`), and the height `5` rode along untouched. The third column being a plain `[0, 0, 1]` is the matrix telling you "z is left alone."

{/* DIAGRAM PLACEHOLDER: a unit cube rotating a quarter turn around the vertical z-axis, with k-hat fixed in place and i-hat, j-hat sweeping around */}

## Worked example: rotation around the y-axis

Change the axis and you get a genuinely different transformation. Rotate a quarter turn around the y-axis instead. Now `j-hat` is the one left alone, and the action happens in the x-z plane. Send `i-hat` to `[0, 0, -1]` (the x direction tips down toward negative z), keep `j-hat` at `[0, 1, 0]`, and send `k-hat` to `[1, 0, 0]` (the z direction swings toward positive x). The matrix:
...
Apply it to `[3, 4, 5]`: `3 · [0, 0, -1] + 4 · [0, 1, 0] + 5 · [1, 0, 0] = [0, 0, -3] + [0, 4, 0] + [5, 0, 0] = [5, 4, -3]`. This time the height `4` along y was the part left untouched, while x and z traded places with a sign flip. Same idea as the z-axis turn, different axis, different fixed direction. In 3D you always have to name the axis.
...
The columns say `i-hat` doubles to `[2, 0, 0]`, `j-hat` triples to `[0, 3, 0]`, and `k-hat` shrinks to `[0, 0, 0.5]`. Apply to `[3, 4, 5]`: `3 · [2, 0, 0] + 4 · [0, 3, 0] + 5 · [0, 0, 0.5] = [6, 12, 2.5]`. Each coordinate just scaled by its own factor: x doubled, y tripled, z halved. A diagonal matrix is the simplest 3D transformation to read, because each axis minds its own business.
...
Work one through. For the scaling matrix above, the three columns are `[2, 0, 0]`, `[0, 3, 0]`, and `[0, 0, 0.5]`, all still pointing along the axes. The unit cube becomes a rectangular box two units wide, three tall, half a unit deep: no slant, just stretched and squashed along each direction. For the z-axis rotation, the three columns stay unit length and perpendicular, so the cube stays a cube and only spins a quarter turn. You read the box straight off the columns, without testing a single interior point.

{/* DIAGRAM PLACEHOLDER: the unit cube before transformation, and the slanted parallelepiped it becomes after, with its three edges from the origin being the three columns of the matrix */}
...
That is worth holding onto when a model has a "768-dimensional embedding space." It means 768 basis directions, vectors with 768 entries, and matrices with 768 columns, behaving by the rules you just used on `[3, 4, 5]`. You cannot sketch the cube anymore, but you do not need to. As the first lesson put it, high-dimensional means long list, not magic. 3D is just the last list you can see.

## Common pitfalls

**Saying "rotation" without naming the axis.** In 2D there was one way to rotate. In 3D you must say which axis you turn around; rotation about z, y, and x are three different transformations with three different matrices.
...
- **A 3x3 matrix is where `i-hat`, `j-hat`, and `k-hat` land, written as three columns,** and `M · v = x · col1 + y · col2 + z · col3`. To sketch its effect, plot the three columns and draw the parallelepiped they span out of the unit cube.
