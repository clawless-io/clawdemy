...
*i-hat* points one unit along x, *j-hat* one unit along y, and the new arrival *k-hat* one unit along z, straight out of the old flat plane toward you. Every 3D vector is a combination of these three: the vector 3, 4, 5 is just 3 times *i-hat* plus 4 times *j-hat* plus 5 times *k-hat*, three steps along x, four along y, five along z.
...
The matrix-vector product extends in the obvious way. For *v* the vector with components *x*, *y*, *z*,
...
The x and y parts rotated just like the old 2D example (the vector 3, 4 became negative-4, 3), and the height 5 rode along untouched. The third column being a plain 0, 0, 1 is the matrix telling you "z is left alone."
...
Each coordinate just scaled by its own factor: x doubled, y tripled, z halved. A diagonal matrix is the simplest 3D transformation to read, because each axis minds its own business.
...
- **A 3x3 matrix is where *i-hat*, *j-hat*, and *k-hat* land, written as three columns,** and the product *M* times *v* equals *x* times column one plus *y* times column two plus *z* times column three. To sketch its effect, plot the three columns and draw the parallelepiped they span out of the unit cube.
