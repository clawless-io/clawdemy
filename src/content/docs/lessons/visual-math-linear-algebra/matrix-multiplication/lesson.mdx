...
Write the product as `AB`. The convention, which feels backward until it clicks, is that `AB` means **first apply `B`, then apply `A`**. The matrix on the right goes first.

This is not arbitrary either. It comes straight from how we already write functions. When you see `f(g(x))`, the inner function `g` runs first, because `x` reaches `g` before it reaches `f`. Matrices work the same way. Applying the product to a vector,

```
(AB) · v  =  A · (B · v)
```

shows it directly: `v` meets `B` first (it is closest), then the result meets `A`. The matrix nearest the vector acts first. Read right to left, always.
...
- **First column of `AB`** is `A` applied to the first column of `B`.
- **Second column of `AB`** is `A` applied to the second column of `B`.
...
The columns of `R` are `[0, 1]` and `[-1, 0]`. Recall that `S` acts as `S · [x, y] = x · [1, 0] + y · [1, 1]`.
...
Check it against doing the steps one at a time on `[3, 4]`. First rotate: `R · [3, 4] = [-4, 3]` (from last lesson). Then shear: `S · [-4, 3] = -4 · [1, 0] + 3 · [1, 1] = [-1, 3]`. Now apply the combined matrix directly: `SR · [3, 4] = 3 · [1, 1] + 4 · [-1, 0] = [3, 3] + [-4, 0] = [-1, 3]`. Same answer. The product matrix really does capture both moves at once.

## Worked example: the other order

Now do the moves in the opposite sequence, shear first then rotate, which is the product `RS`. Apply `R` to each column of `S`. The columns of `S` are `[1, 0]` and `[1, 1]`, and `R · [x, y] = x · [0, 1] + y · [-1, 0]`.
...
Apply it to `[3, 4]`: `RS · [3, 4] = 3 · [0, 1] + 4 · [-1, 1] = [0, 3] + [-4, 4] = [-4, 7]`.

Compare the two results. Rotate-then-shear sent `[3, 4]` to `[-1, 3]`. Shear-then-rotate sent it to `[-4, 7]`. Different points, different matrices.
...
There is one thing matrix multiplication keeps from ordinary arithmetic: it is **associative**. For three transformations, `(AB)C = A(BC)`. You may group the chain however you like, as long as you do not change the left-to-right order.

Take `A` = a horizontal stretch by 2, `B = R` the rotation, `C = S` the shear:
...
Group it as `(AB)C`. First `AB`: apply `A` to the columns of `B`, giving columns `A · [0, 1] = [0, 1]` and `A · [-1, 0] = [-2, 0]`, so `AB = [[0, -2], [1, 0]]`. Then `(AB)C`: apply `AB` to the columns of `C`, giving `(AB) · [1, 0] = [0, 1]` and `(AB) · [1, 1] = [-2, 1]`. Result: `[[0, -2], [1, 1]]`.

Now group it as `A(BC)`. First `BC`: apply `B` to the columns of `C`, giving `[0, 1]` and `[-1, 1]`, so `BC = [[0, -1], [1, 1]]`. Then `A(BC)`: apply `A` to those columns, giving `[0, 1]` and `[-2, 1]`. Result: `[[0, -2], [1, 1]]`.

Same matrix both ways. The reason is not arithmetic luck: "apply `C`, then `B`, then `A`" is one fixed sequence of moves, and where you put the parentheses only changes which two adjacent moves you bundle up first, never the order they happen in. That is why we can write `ABC` with no parentheses at all.
...
**Reading the product left to right.** `AB` applies `B` first. The rightmost matrix is closest to the vector and acts first, exactly like the inner function in `f(g(x))`. If you apply them left to right, you get the wrong order and usually the wrong answer.

**Assuming you can swap the order.** `AB` is generally not `BA`. Order is the sequence of physical moves, and rotating-then-shearing differs from shearing-then-rotating. Only in special cases do two transformations happen to commute.

**Confusing not-commutative with not-associative.** Order of operations matters (`AB ≠ BA`), but grouping does not (`(AB)C = A(BC)`). These are different statements. You may not reorder the chain, but you may parenthesize it any way you like.
...
- **Multiplying matrices means composing transformations: `AB` is "do `B`, then `A`."** Read right to left, like `f(g(x))`. The product is a single matrix recording where the basis lands after both moves.
...
- **Order matters, grouping does not.** `AB ≠ BA` in general (rotate-then-shear is not shear-then-rotate), but `(AB)C = A(BC)` always, because the chain of moves is one fixed sequence no matter how you parenthesize it.
