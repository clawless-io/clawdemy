...
An `m × n` matrix has `m` rows and `n` columns, and it maps `n`-dimensional input to `m`-dimensional output. The cleanest way to keep this straight is to read it through the columns, exactly as before:
...
Two columns, three rows: it takes a 2D vector in and gives a 3D vector out. The columns are `[1, 0, 1]` and `[0, 1, 1]`, the landing spots of `i-hat` and `j-hat`, now living in 3D. Apply it to `[3, 4]`:
...
Three columns, two rows: it takes a 3D vector in and gives a 2D vector out. The columns are `[1, 0]`, `[0, 1]`, and `[0, 0]`. Apply it to `[3, 4, 5]`:
...
But something did get destroyed, and the null space names it. Any input of the form `[0, 0, z]`, anywhere along the z-axis, gets sent to `[0, 0]`. The entire z-axis is crushed to the origin, so the null space is that z-axis, a 1D line, and the nullity is 1. The transformation cannot tell two points apart if they differ only in height.
...
A 3x2 matrix again, so 2D in, 3D out. But its columns are `[1, 2, 3]` and `[2, 4, 6]`, and `[2, 4, 6] = 2 · [1, 2, 3]`: dependent. They do not span a plane, only the line through `[1, 2, 3]`. So the rank is 1, and the column space is a 1D line in 3D output space. For the null space, solve for the inputs sent to zero: every row reduces to `x + 2y = 0`, so `x = -2y`, giving the line through `[-2, 1]` in the 2D input. Nullity 1. Rank 1 plus nullity 1 equals 2, the input dimension, and the books balance again. This matrix collapses its 2D input down to a single line in 3D, the rectangular version of the determinant-zero collapse from earlier.
...
Run it once on a fresh matrix. Take `[[2, 0, 1], [0, 2, 1]]`: three columns, two rows, so it maps 3D input to 2D output, a projection. Its columns `[2, 0]`, `[0, 2]`, and `[1, 1]` include two that already span the full 2D output, so the rank is 2, full. Being a projection, it has to crush something: solving for the inputs sent to zero gives `2x + z = 0` and `2y + z = 0`, so `x = y` and `z = -2x`, the line through `[1, 1, -2]`. Nullity 1, and rank 2 plus nullity 1 is 3, the input dimension. Four readings, one matrix.
...
**Reading the shape backward.** An `m × n` matrix maps `n`-dimensional input to `m`-dimensional output. The column count is the input dimension, the row count is the output dimension. When in doubt, count the columns: that is how many basis vectors the input has.
...
- **An `m × n` matrix maps `n`-dimensional input to `m`-dimensional output:** `n` columns (one per input basis vector), each an `m`-dimensional landing spot. The shape tells you the direction of the mapping.
...
- **The rules carry over unchanged.** Rank is the dimension of the column space (in the output); the null space lives in the input; and `rank + nullity = number of columns` still balances exactly.
