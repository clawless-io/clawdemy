...
**The generic case: the whole plane.** Pick two vectors that point in genuinely different directions, like the vectors 1, 0 and 0, 1, or almost any random pair. By dialing *a* and *b* up, down, and negative, you can reach every single point in 2D space. The span is the entire plane.
...
Now reach the same point with a different basis, the vectors 1, 1 and 1, negative-1. You need scalars *a* and *b* that combine them to land on the point 5, 3:
...
**Forgetting the origin is always in the span.** Choosing *a* equal to 0 and *b* equal to 0 gives the zero vector, so the origin is reachable from any set of vectors. Every span passes through the origin. This is why spans are lines and planes through the origin, never lines or planes off to the side.
