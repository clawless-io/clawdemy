...
The `-c` means "create the branch and switch to it in one step." Without `-c`, `git switch` switches to an existing branch.
...
The `-b` means "create and switch." Without `-b`, `git checkout` switches to an existing branch (or, confusingly, to a file or commit).
...
This two-step form is useful when you want to mark a point in history without leaving your current branch. Most of the time, the `-c` / `-b` shortcut is what you want.
...
(We cover remote-tracking branches properly in L8. For now, know that `-a` shows you "all branches I know about, including the ones the remote server has.")
...
If the branch has work that hasn't been merged into your current branch, `-d` refuses. This is git protecting you from accidentally losing work.
...
Use `-D` (capital D) when you know what you are doing. The commits don't actually disappear (they live in reflog for 90 days per L4), but the branch name is gone.

**Discipline:** prefer `-d` by default. The git error message that `-d` produces ("branch is not fully merged") is a useful warning. If you really do want to delete unmerged work, take the half-second pause to type `-D` deliberately.
...
The `<type>/<short-description>` pattern serves two purposes:
1. **Filterable lists.** `git branch | grep feature/` shows all features.
2. **Self-documenting URLs.** Pull request URLs include the branch name, making the PR list scannable.

Some teams use `username/feature-name` (for example, `jay/payment-flow`) to disambiguate. Some teams enforce conventions via CI. Most teams converge on something close to the type-prefix pattern within a few months of starting to use PRs.
...
**Second**, branch hygiene predicts team velocity better than most metrics. Teams whose `git branch -a` lists hundreds of stale branches usually have process gaps: PRs are not getting reviewed promptly, deletions are not happening after merge, abandoned experiments are not getting cleaned up. The branch list is a leading indicator. If your engineering team's branch list is growing without bound, that is a signal worth surfacing in a 1:1.
...
- Create a branch with `git switch -c <name>` (or `git checkout -b <name>`)
