...
- `<<<<<<< HEAD` is the start of "your" version (whatever branch you're currently on)
- `=======` is the separator between the two versions
- `>>>>>>> feature/cart-refactor` is the end of the other version, with the other branch's name

**Reading the markers:**
- Everything between `<<<<<<< HEAD` and `=======` is your branch's version of these lines
...
3. You stage the file with `git add <file>` to tell git "I've resolved this one"
...
The "Unmerged paths" section lists every file with a conflict. `both modified` means both branches changed the file in conflicting ways.

**Step 2: Open each conflicting file and find the markers.**

Search for `<<<<<<<` (or use your editor's built-in "next conflict" navigation). Each marker block is one conflict region. A single file might have multiple conflict regions; each one independently needs resolving.
...
Delete the marker lines (`<<<<<<<`, `|||||||`, `=======`, `>>>>>>>` lines, all of them). Write the resolved content where the conflict was. The result should look like normal code, no leftover markers.
...
You're on `feature/login-redesign`. Your teammate updated the same page's heading on main. You both changed `<h1>` to different things.
...
`git add login.html` and `git commit`. Done. Total time: 30 seconds once you've practiced.

### Worked example 2: Logical combine (both changes needed)

You're on `feature/cart-improvements`. You added support for discount codes. Your teammate on main added support for gift cards. Both changes touched the same `calculateTotal` function.
...
`git add cart.ts`. Then before committing, write a test that covers a cart with BOTH a discount code and a gift card balance. The test is what proves you combined them correctly. Then commit.
...
You're on `feature/auth-cleanup`. You renamed the function `authenticateUser` to `signIn` for consistency with the rest of the codebase. Meanwhile on main, your teammate added a new parameter (`requireMfa: boolean`) to the old `authenticateUser`.
...
- Take their version for now (`git add`)
...
You're on `feature/restructure-utils`. You moved `src/utils/format.ts` to `src/lib/format.ts`. Meanwhile on main, your teammate edited the contents of `src/utils/format.ts` (in the old location).
...
1. Look at what they changed in `src/utils/format.ts` (use `git show main -- src/utils/format.ts`)
2. Apply the same edits to `src/lib/format.ts` (manually)
3. `git rm src/utils/format.ts` to remove the old path
4. `git add src/lib/format.ts` to confirm the new path with their edits
...
**Command-line `git mergetool`.** Launches an external merge tool you've configured. Many developers go a whole career without using it; the built-in markers + an editor are usually enough.
...
**The pattern:** `git merge --abort` is not failure. It is taking a do-over after you've gathered information you didn't have before. Free yourself to use it.
...
**2. The merge can be undone.** `git merge --abort` takes you back to exactly where you were. No matter how badly you mess up the resolution, you can start over. There is no "ruined" state.
...
- Recognize when git is in a merge state (`git status`)
...
- Choose between aborting (`git merge --abort`) and resolving forward
