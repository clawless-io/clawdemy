---
title: 'Lesson: Undoing things'
description: The lesson that closes Phase 1. You learn to recover from mistakes safely. Discard working changes. Unstage staged changes. Undo commits without losing work. The reflog as your safety net. By the end you have confident solo git workflow.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
5. You run something destructive (`git reset --hard` is the classic), then look in horror at what just disappeared. You want it back.
...
|---|---|---|
| 1. Discard working changes | Working directory | `git restore <file>` |
| 2. Unstage a staged file | Staging area | `git restore --staged <file>` |
| 3. Fix the last commit | Repository (last commit only) | `git commit --amend` |
...
| 6. Recover from a destructive operation | Reflog (god mode) | `git reflog` + `git reset --hard <hash>` |
...
You opened `notes.md`, made some edits, and now want to throw them away. The file should look like its most recent committed state.
...
### Scenario 2: Unstage a file (`git restore --staged`)

You ran `git add notes.md`. Now you realize you do not want it in the next commit. You want to unstage it without losing your edits.
...
If you have been using git for a while, you may also have seen `git reset HEAD <file>` for this. That command still works and does the same thing. `git restore --staged` is the modern, more explicit version that git's documentation now recommends. Either works; use whichever your hands remember.

### Scenario 3: Fix the last commit (`git commit --amend`)
...
The `--no-edit` keeps the original commit message. Without it, git opens your editor to let you edit the message too.

**Important: `--amend` rewrites history.** The "amended" commit has a different SHA-1 hash from the original (git treats it as a brand new commit that happens to look similar). If the original commit was already pushed to a shared branch, amending creates a divergence between your local history and the remote. Don't amend pushed commits unless you are sure no one has pulled them.

The rule of thumb: `--amend` is fine for commits that are still local. Once pushed, treat the commit as immutable.
...
**Mode 1: `--soft`, undo the commit, keep everything in staging.**
...
**Mode 2: `--mixed` (the default), undo the commit, keep changes in working directory.**
...
**Mode 3: `--hard`, undo the commit AND throw away the changes.**
...
**`--hard` is the destructive one.** Use it deliberately. Pause and run `git status` before, so you know what is in your working directory that you might accidentally lose. The files are not unrecoverable (we will get to `git reflog` in a minute), but the recovery takes effort.

**`HEAD~1` means "one commit before HEAD."** `HEAD~3` means three before. `HEAD` alone means "the current commit." So:
- `git reset HEAD~3` undoes the last three commits
- `git reset --hard HEAD~3` undoes the last three commits and discards their working changes
...
- `abc1234` is the commit you want to undo
- Git opens your editor with a pre-filled message (`Revert <original commit message>`); accept or edit, then save and close
- A new commit lands on your branch, undoing `abc1234`'s changes
...
You ran `git reset --hard`. Or you force-pushed the wrong branch. Or you ran `git checkout <old-branch>` and lost track of where you were. Your commits seem to have vanished.
...
Each line is a previous HEAD position. The hashes on the left are the commit SHAs at each point. To recover:
...
- `commit --amend` rewrites the last commit (and nothing else)
...
- Unstage files from staging (`git restore --staged`)
- Fix the last commit's message or contents (`git commit --amend`)
- Undo a local-only commit with `git reset` (soft / mixed / hard)
