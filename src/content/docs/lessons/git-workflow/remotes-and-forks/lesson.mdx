...
The `-v` flag means "verbose"; it shows the URLs. Without `-v`, you'd just see the name `origin`.
...
The `-u` (short for `--set-upstream`) tells git to remember the relationship between your local branch and the remote branch. After running this once, future `git push` calls on this branch work without arguments.
...
- Updated `origin/<branch>` references (the remote-tracking branches)
...
`git pull` is a shortcut. It runs `git fetch` followed by `git merge origin/<branch>`. So `pull` = fetch + merge.
...
After this setting, every `git pull` automatically uses `--rebase`. Most modern teams prefer this; pull-rebase keeps history clean and avoids the cluttered "Merge branch 'main' into 'main'" commits that simple pull produces.

We'll dive deeper into rebase in L12. For now, know that `--rebase` is usually the better default for `pull`.

### Remote-tracking branches: the labels for "what the remote looked like"

When you fetched, git updated something called `origin/main` (and `origin/feature-x` and so on). These are **remote-tracking branches.**
...
These are not the same as your local branches. Your local `main` might be ahead of, behind, or diverged from `origin/main`. The whole point of `git fetch` is to update the `origin/X` labels so you can see what the remote has without integrating it.
...
The "ahead / behind / diverged" framing is exactly the relationship between your local branch and `origin/<branch>`, the remote-tracking label.
...
The branch now exists on the remote. Your teammates can see it (`git fetch` then `git branch -r`). They can check it out locally (`git checkout feature/payment-flow`). They can contribute to it (push their own commits). The branch is now a shared resource, not just a local thing.
...
On the GitHub web UI, click "Fork" on the original `withastro/astro` repo. GitHub creates `your-username/astro`, a copy of the original repo under your account. You have full write access to your fork.
...
Now you have a local copy. Your remote `origin` points to your fork (`your-username/astro`).
...
The branch now exists on `your-username/astro`.

**Step 6: Open a PR from your fork to the original.**

On GitHub, navigate to `withastro/astro`. GitHub detects the recent push to your fork and offers a banner: "Compare & pull request." The PR is from `your-username:fix/broken-config-loader` to `withastro:main`.

**Step 7: Sync with upstream as their main moves.**

Days or weeks pass. The maintainers merge other PRs into `withastro/astro` main. Your fork is now behind. You sync:
...
Maintainer reviews. Maybe asks for changes (L6 etiquette). You push fixes. Maintainer approves and merges. Your contribution is now in `withastro/astro`.
...
The fork model uses two remotes (`origin` + `upstream`). Some workflows use more.
...
**Mirror/backup remote:** some teams add a third remote pointing to a backup server. `git remote add backup git@backup.internal:project.git`. They push to both `origin` (the team server) and `backup` (the backup). Belt and suspenders.

**Multi-fork collaboration:** you and another contributor are co-developing a PR. They have their fork; you have yours. You add their fork as a remote (`git remote add coworker https://github.com/their-username/project.git`) so you can fetch their branches and contribute to their PR.
...
Most developers spend most of their time with just `origin` or `origin` + `upstream`. The N-remote workflows are niches. Recognize them when you see them; you don't need to use them daily.
...
This protects you from accidentally overwriting a teammate's work. If your last `git fetch` saw the remote at commit X, and the remote is still at X, force-with-lease succeeds. If the remote is at Y (because a teammate pushed), force-with-lease refuses and you can fetch + decide.
...
- Shared branches like `main`, `develop`, `release/*`
...
### The `git pull --rebase` setting: make this your default

The default behavior of `git pull` is `fetch + merge`. This creates merge commits when you pull updates. Over a year of pulls, your history accumulates dozens of "Merge branch 'main' of github.com:..." commits, visual noise that nobody benefits from.

The alternative: `git pull --rebase` runs `fetch + rebase`. Your local commits get replayed on top of the fetched commits. No merge commit. Linear history.
...
Now `git pull` automatically uses `--rebase`. Most modern teams set this; many recommend it as the first config a new developer adds.

**One caveat:** if you have UNCOMMITTED changes, `git pull --rebase` refuses (because rebase can't replay changes that aren't commits). The fix: stash your changes (`git stash`), pull, then unstash (`git stash pop`). Or just commit your work first.
...
If you came from Mercurial, the model is nearly identical. `hg push` and `hg pull` work analogously to `git push` and `git pull`. The main difference is that Mercurial doesn't have remote-tracking branches as a separate concept; it tracks remote state more implicitly. Git's explicit `origin/main` labels are slightly more complex but give you more visibility.
...
- Daily `git pull --rebase` on main to stay current.
...
What changes:
- Multiple long-lived branches (`main`, `develop`, `release/*`). Engineers need to push to the right one.
...
- The `pull --rebase` default becomes a team convention to keep history clean.
...
The fork-based model from Worked Example 3 above. `origin` = your fork. `upstream` = the original. Push to origin, PR to upstream. This is how the global open-source ecosystem operates. Anyone reading the L8 material can submit a PR to almost any open-source project on GitHub by tomorrow.

The discipline: rebase on upstream frequently so your PR stays mergeable. Force-with-lease only on your own branches. Sync your fork's `main` from `upstream/main` weekly even when you don't have an active PR.
...
**3. The reflog (L4) catches local mistakes.** Even if a `git pull --rebase` produces a confusing result, the reflog shows where you were before, and `git reset --hard <hash>` from there gets you back. The safety net from L4 still works.
...
- Push commits to a remote with `git push -u origin <branch>` (first time) or `git push` (thereafter)
- Fetch updates without merging via `git fetch`
- Pull updates with `git pull` or (preferably) `git pull --rebase`
- Set up the fork model with `origin` + `upstream` remotes
- Recognize remote-tracking branches (`origin/main`) and what they tell you
- Use `git push --force-with-lease` safely on your own branches
- Configure `pull.rebase = true` as your default
