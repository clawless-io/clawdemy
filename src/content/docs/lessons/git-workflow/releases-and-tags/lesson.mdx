...
A **tag** is a named pointer to a specific commit that never moves. Unlike a branch (which moves forward when you commit), a tag is fixed. Once you tag commit `abc1234` as `v1.0`, the tag `v1.0` always means commit `abc1234`. Forever (or until you delete the tag).
...
Tags don't push with `git push` by default. You have to push them explicitly:
...
Most modern git configurations auto-fetch tags during regular `git fetch`. The explicit form is for when they don't.
...
- `v1.0.0` -> `v1.0.1`, bug fix (PATCH bump)
- `v1.0.1` -> `v1.1.0`, new feature, backward compatible (MINOR bump)
- `v1.1.0` -> `v2.0.0`, breaking change (MAJOR bump)
- `v2.0.0` -> `v2.0.1`, bug fix in the new major version (PATCH bump)
...
**Build metadata** (optional): `v2.0.0+build.20260610.abc1234` adds machine-readable build info after a `+`. Rarely needed except for build pipelines.
...
- Often the release process is documented in `CONTRIBUTING.md` so the community knows the cadence ("we release on the first Tuesday of each month").
...
1. Configure `.github/release.yml` to define categories and how to populate them.
...
If you came from Mercurial, hg tags work similarly but are stored as commits in `.hgtags`. Git tags are stored as ref files (`.git/refs/tags/`). The conceptual model is the same; the implementation is cleaner in git.
...
The Clawless 2026-06-04 sprint pattern (six agents, Lead integrating, founder reviewing) implicitly created a release at the end of each track. Each track promotion to production was effectively a release event. The discipline of formally tagging it (`clawdemy-v2026.06.04`) would have provided a marker for "this is what the sprint produced."
...
**2. Wrong releases are recoverable.** If you tag the wrong commit, delete the tag (`git push origin --delete vX.Y.Z`) and tag the right one. If the release shipped a critical bug, ship vX.Y.Z+1 with the fix. Nothing is permanent.
...
- Create annotated tags with `git tag -a vX.Y.Z -m "message"`
- Push tags to a remote with `git push origin vX.Y.Z` or `--tags`
