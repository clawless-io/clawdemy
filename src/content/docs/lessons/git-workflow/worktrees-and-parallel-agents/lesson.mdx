...
You're mid-feature on `feature/payment-flow`. Three files modified, nothing committed. The work is going well. You'd like to keep going.
...
You now have a folder `../review-dashboard` (next to your current repo folder) with `feature/dashboard-redesign` checked out. You can `cd ../review-dashboard`, run the dashboard project, test it, leave a review. Your original folder is untouched: `feature/payment-flow` is still there, files still modified, exactly as you left it. When you're done with the review:
...
1. The **git directory** (the `.git/` folder), which holds all the history, branches, tags, configuration. The "database."
...
`<path>` is where the new working tree's folder will live. Relative paths are relative to your current repo's root. `<branch>` is the branch you want checked out in the new worktree.
...
A new folder `../review-dashboard` is created. `feature/dashboard-redesign` is checked out there. If the branch existed only on the remote, git fetches it.
...
A new folder `../new-feature` is created. A new branch `feature/new-thing` is created off `main` and checked out there.
...
Deletes the worktree's folder and removes git's record of it. If the worktree has uncommitted changes, git refuses; use `--force` to override (or commit/stash the changes first).
...
If you manually deleted a worktree folder (e.g. `rm -rf`) without running `git worktree remove`, git still has a record of it. Prune cleans up these stale records.
...
A more familiar comparison: when you switch branches via `git checkout <other-branch>`, your single working tree gets reset to the other branch's files. Any uncommitted changes either come along (if compatible) or block the checkout (if conflicting).
...
Worktrees eliminate this friction by giving each branch its own working tree. You `cd` to switch context. No file shuffling. No risk of losing uncommitted state.
...
Total time: 30 seconds of overhead. Your `myrepo` folder was untouched during the review. Files still modified, exactly as before.
...
Your `myrepo` folder still has the feature work. The hotfix happened in a parallel space. The two never touched.
...
Now in one terminal, you can be in `myrepo` with `main` and in another terminal in `check-v1.4` with `v1.4`. Run both. Diff outputs. Find the regression-introducing commit.
...
Each agent operates in its own folder, on its own branch. They commit independently. Their commits flow into the shared `.git/` so the lead (you, or another orchestrator) can see all of them with `git log --all --oneline`.
...
- In tab 3, run `git tag -a v1.4.3 -m "fix: critical bug X"`, push the tag, draft release notes
...
The commits all live in the shared `.git/`. You can read each branch's progress without leaving the main repo. You can see disagreement (two agents both editing the same file unaware), conflict potential (two branches trying to add similar code), and progress (which agent is moving faster).
...
Use `--force` when you're sure the changes don't matter. Note: untracked files in the worktree ARE deleted by force-remove. There's no recovery for true untracked files (no reflog for never-added files).
...
The tests run in `../test-feature-X` using its own files. Your `myrepo` folder is untouched. You continue developing. When the tests finish 45 minutes later, you check the results in `../test-feature-X`.
...
**Submodules in worktrees.** Worktrees and submodules play together but with caveats. If your repo uses submodules, the submodule state is per-worktree (each worktree gets its own submodule checkout). Refresh submodules when you `worktree add`.
...
**Disk space.** Each worktree includes a full working tree (all the files). For large repos, this can be significant. Worktrees still save disk space compared to clones (which would also duplicate `.git/`), but they're not free.

**Switching default tree.** The original repo's working tree (the one with the `.git/` folder inside it) is sometimes called the "primary" worktree. You can't remove this one via `git worktree remove`. To "move" your primary worktree, you'd `git clone` fresh or use `git worktree move` (which exists but is for edge cases).
...
If you're coming from Mercurial: `hg share` is the closest analog (shared store, multiple working directories). Mercurial historically also had `hg multiclone` and external tools. The git worktree feature is cleaner and more first-class.
...
The general principle: git worktrees are unusually elegant because the database (the `.git/`) is genuinely shared (no duplication, no sync overhead) while the working trees are completely independent. Other VCSs either duplicate more or share less.
...
- Share git history (commits flow into shared `.git/` immediately, no fetch ceremony)
...
- Create a worktree with `git worktree add ../path branch` and use it as an independent working directory
- List your worktrees with `git worktree list` to see what's currently attached
- Remove a worktree with `git worktree remove ../path` (or `--force` if it has uncommitted changes you don't care about)
