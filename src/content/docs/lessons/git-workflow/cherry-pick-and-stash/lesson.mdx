---
title: 'Lesson: Cherry-pick and stash'
description: Two surgical tools every working developer reaches for. Cherry-pick copies a single commit from one branch onto another, the canonical way to backport hotfixes. Stash saves in-progress work without committing, the safety net for context switches. By the end of L11 you can backport a fix across branches, set aside dirty work to handle an emergency, and recover safely from both.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
Then your phone buzzes. It's your team lead. "Hey, that fix needs to go onto `release/v1.4` too. We're still supporting v1.4 for the enterprise customers who haven't upgraded yet. Can you backport it?"

What do you do? You could re-implement the fix by hand on `release/v1.4`. You could merge `main` into `release/v1.4`, but that drags in fifty other commits you don't want on the release branch. You could create a new branch off `release/v1.4` and try to remember exactly what you changed.
...
Where `abc1234` is the SHA of that one fix commit on `main`. Cherry-pick takes that single commit's diff and applies it to your current branch as a new commit. The fix lands on `release/v1.4`. You push. You're done.
...
Take a commit on branch A. Its diff says "change line 47 from X to Y in file `foo.py`." Cherry-pick on branch B replays that diff on top of branch B's current state, producing a new commit on B with the same author info and message but a different SHA.
...
Applies all commits from (but NOT including) abc1234 up to and including ghi9012. The exclusive-start form is a common source of off-by-one confusion; if you want abc1234 included, use `abc1234^..ghi9012` (the caret means "the parent of," so the range starts just before abc1234).
...
The `-x` flag adds a line to the new commit message: `(cherry picked from commit abc1234)`. This is gold for debugging six months later when someone asks "where did this commit come from?" Some teams require `-x` on all cherry-picks for exactly this reason.
...
- **Hotfix backport.** Fix is on `main`, you need it on `release/v1.4` too. Cherry-pick the fix commit onto the release branch.
- **Forward-port from a release branch.** A fix went to `release/v1.4` first (the LTS branch). It also needs to be on `main` going forward. Cherry-pick it forward.
...
- **Picking from another developer's branch.** Their PR is still in review but you need one specific commit now. Cherry-pick it onto your branch (with their permission and the `-x` flag for traceability).
...
Working tree resets to clean. Stash goes on the stack as `stash@{0}`.
...
The message shows in `git stash list` so you can identify stashes later. Without messages, you end up with "WIP on main: abc1234 some commit message" auto-generated names that don't tell you anything.
...
By default, stash only catches files git already knows about. Untracked files (new files you haven't `git add`-ed yet) get left in your working tree. The `-u` flag scoops them up too. Use this when you've created new files that you want stashed with the rest.
...
The branch shown is the branch you were on WHEN you stashed. The number in `stash@{N}` is the stack position (0 = most recent).
...
Without `-p`, you get a summary (which files changed, how many lines). With `-p`, the full diff. Useful when you're not sure if a stash is still relevant.
...
Restores `stash@{0}` to your working tree AND deletes it from the stack. The default. Use this when you're confident the restore will work cleanly.
...
Useful when you want to try restoring and not lose the stash if something goes wrong. After confirming, you can `git stash drop stash@{0}` to remove it.
...
- **Lost work.** A `git stash drop` or `git stash clear` is essentially deletion. If you stashed something important and dropped it, recovery is hard (possible via the reflog, but requires comfort with `git fsck --lost-found`).
...
You're mid-feature on `feature/dashboard`. Two files modified, nothing committed yet. Your team lead pings: "Production is down. A nullpointer in the auth flow. Can you fix it?"
...
You merged a fix to `main`. The fix needs to also land on `release/v1.4` for the customers still on that release.
...
**Setup:** You're maintaining a library with three active versions: `main` (3.x development), `release/v2.4` (LTS, 24-month support), `release/v1.8` (LTS, last 6 months of support). A security bug is found that affects all three.
...
Open a PR against `release/v2.4`. Get review (a peer who knows the v2.4 codebase). Merge.
...
**Setup:** You're on `feature/payment-receipts`. You've been editing `templates/receipt.html` and `services/email.py`. Nothing committed.
...
**Setup:** You spent two days on `experiment/new-cache-layer`. You wrote five commits. You explored an idea and decided the overall architecture isn't right. The branch is going to be deleted.
...
`stash branch` creates a new branch starting from the commit you were on when you stashed (three weeks ago), applies the stash there, and drops it. You now have a clean branch with your work in its original context. You can rebase or merge from main at your own pace.

### A note for experienced developers

If you're coming from Mercurial: `hg graft` is the equivalent of `git cherry-pick`. Same concept (copy a commit's diff onto your current branch), same conflict-handling pattern. `hg shelve` (extension) is the equivalent of `git stash`. The mental model transfers directly.

If you're coming from SVN: there is no SVN equivalent of cherry-pick. SVN's "merge a range" is the closest, and it's a much heavier operation. SVN users coming to git often discover cherry-pick and immediately fall in love with it for the precision. Stash also has no SVN equivalent; SVN users either committed half-finished work to a branch or used patches (`svn diff > my.patch`, then revert, then apply later).
...
- **Backports need their own QA.** A fix that passed tests on `main` might not behave identically on `release/v1.8` because the surrounding code differs. Each backport needs its own validation pass.
...
- **Cherry-pick traceability matters for audit.** Use `-x` (or a project convention that links new to original commits) so when an auditor asks "how did this fix get to the v1.8 release?" the answer is in the commit message.
...
Cherry-pick traceability with `-x` is usually a team convention or required by code review. Backports have their own PR templates that reference the original fix.
...
The thing to internalize: both are reversible. `git cherry-pick --abort` undoes a cherry-pick mid-conflict. `git stash drop` removes a stash without applying it. `git reset HEAD~1` undoes a cherry-pick that already committed. Nothing about either command is destructive in a way that can't be recovered (the reflog has your back for 90 days).
