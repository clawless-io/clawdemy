---
title: 'Lesson: Rebase, deeper'
description: A deep treatment of rebase. Interactive rebase for history cleanup (pick, reword, squash, fixup, drop, reorder). The rebase-vs-merge decision under pressure. The hard rule about rebasing published commits and what happens when you break it. How to recover from a rebase gone wrong. By the end of L12 you can clean up a feature branch's messy commits into a reviewable story, choose rebase vs merge with confidence, and back out of any rebase safely.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
The tool that lets you do that is **interactive rebase**. `git rebase -i`. The "i" stands for interactive, and what it does is let you replay your branch's commits one at a time with the chance to edit, combine, drop, reword, or reorder each one along the way.
...
If a replayed commit conflicts with main, rebase pauses. You resolve, `git add`, then `git rebase --continue` (or `--abort` to give up).
...
The `HEAD~5` part means "the commit 5 back from where I am now." So this command says: "Take the last 5 commits and let me edit them as I replay them."
...
**`edit`** (or `e`): Replay the commit, but pause after replaying and let you make changes to the working tree (add files, change the diff). Then you `git commit --amend` and `git rebase --continue`. Use this to retroactively fix what's IN a commit (e.g. you forgot to include a file).
...
**`break`** (or `b`): Pause the rebase here for any reason. You can poke around, run commands, then `git rebase --continue` when ready.
...
(You can also do this in finer-grained pieces by using `edit` to retroactively split a commit, but for most cleanup, squash + reword is enough.)
...
The idea: as you work, when you realize a previous commit needs a small tweak (you forgot a comma, missed a variable rename), don't make a new commit called "fix typo from earlier commit." Instead, make a commit using `--fixup`:

```bash
git commit --fixup=abc1234
```

Where `abc1234` is the SHA of the commit you're tweaking. Git creates a commit with the message `fixup! <original commit message>`. The `fixup!` prefix is a marker.
...
This is the most efficient cleanup pattern once you internalize it. The cost is one extra flag (`--fixup`); the benefit is no last-minute cleanup sprint before opening the PR.
...
Now `git rebase -i main` automatically does autosquash (without the explicit flag). Many developers consider this a near-required config.
...
Three options:
- Resolve the conflict in the file, then `git add` and `git rebase --continue`.
- `git rebase --skip`: skip this commit entirely (its changes are lost).
- `git rebase --abort`: give up the whole rebase, return to the pre-rebase state.
...
- You force-push (`git push --force`), overwriting the remote with your new history, and your teammates' next pull fails or, worse, succeeds but their local work is now based on commits that no longer exist on the remote. They have to recover by hand.
...
- Your feature branch that you've pushed but nobody else has pulled: safe, but use `--force-with-lease` (not plain `--force`) to be defensive.
...
When you DO need to force-push a rebased feature branch (because you cleaned up history), use `--force-with-lease` instead of `--force`:
...
`--force` doesn't check. It just overwrites. If your teammate just pushed a critical fix to your shared feature branch and you force-push your rebase, their fix is gone (recoverable via reflog, but a panicky few minutes).

The discipline: if you find yourself reaching for `git push --force`, stop and reach for `git push --force-with-lease` instead. The extra check costs nothing in the normal case and saves your teammate when something unusual is happening.
...
The line `HEAD@{5}` is "before the rebase started." To return to that exact state:
...
**Caveat:** `git reset --hard` is destructive of WORKING-TREE changes (it discards anything not committed). Don't run it with uncommitted work you care about. If you have uncommitted work, stash it first (`git stash`).
...
**Setup:** You've been working on `feature/customer-search` for two days. `git log feature/customer-search --oneline` shows seven commits:
...
**Step 5:** Done. `git log feature/customer-search --oneline`:
...
**Setup:** You've been on `feature/X` for a week. `main` has moved forward by 60 commits. You want to catch up.
...
You open `src/billing.py`. You see conflict markers. The conflict is between YOUR commit's change and main's recent refactor of the billing module. You resolve carefully (perhaps consulting a teammate familiar with the recent refactor), then:
...
If the rebase had been too painful (say, twenty conflicts because main refactored half the codebase), you might have aborted (`git rebase --abort`) and chosen a different approach: merge main into your branch instead (preserves the linear story of when the merge happened, accepts a merge commit in the history).
...
You realize: you rewrote main's history. Even before pushing, you've broken local main. If you push (`git push --force`), you'll break it for everyone.
...
The simplest recovery: someone who has the old history locally `git push --force-with-lease` to restore the old history. Everyone else pulls the restored version. Anyone who pulled the broken version in between needs to reset their main to the restored remote.

This is a few hours of confusion at minimum, and a real chance someone loses work in the chaos. Avoid getting here. Use branch protection on main (most platforms support "no force-push" rules). Use `--force-with-lease` instead of `--force`. Triple-check the branch before rebasing.

### A note for experienced developers

If you're coming from Mercurial: `hg histedit` is the equivalent of `git rebase -i` (same concept, same actions). Mercurial's `hg rebase` is the equivalent of plain `git rebase`. The mental model transfers directly. The published-history rule is similar: Mercurial's "public phase" prevents rewriting commits that have been pushed.

If you're coming from SVN: there's no SVN equivalent of rebase. SVN history is immutable from the developer's perspective. Cleaning up before commit means using local patches (`svn diff > my.patch`, edit, re-apply) which is much heavier. The git ability to freely rewrite local history is one of the things SVN users most appreciate when they switch.
...
**One: the reflog is your safety net.** No rebase action is permanent. The reflog records every commit your HEAD has touched, even ones that aren't reachable from any branch anymore. If you do something wrong, `git reflog` shows you every state HEAD was in, and `git reset --hard HEAD@{N}` returns to any of them. This is true for 90 days by default (the reflog's expiry). You essentially can't lose work to a rebase mistake unless you wait three months before noticing.
...
- Use `git rebase -i HEAD~N` to clean up your local commits before opening a PR
- Apply `pick`, `reword`, `squash`, `fixup`, `drop`, and reordering to turn messy WIP commits into clean reviewable history
- Use `git commit --fixup=<sha>` during development and `git rebase -i --autosquash` at cleanup time to maintain clean history as you work
- Resolve rebase conflicts the same way as merge conflicts (markers, resolve, `git add`, `git rebase --continue`)
- Recover from a rebase gone wrong with `git rebase --abort` (mid-rebase) or `git reset --hard HEAD@{N}` from the reflog (post-rebase)
