---
title: 'Lesson: Team workflows: GitHub Flow, GitFlow, Trunk-based, Forking'
description: The four production team workflows that build on the primitives from Phase 2. You learn what each one prescribes, when it fits, and how to choose. You see the role of branch protection rules and CI gates. By the end you can read any company's git workflow documentation and know what's prescribed versus optional, including how to set up your own open-source project for outside contributions.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
- Three short-lived branch types: `feature/*` (off develop), `release/*` (off develop, merge to main + develop), `hotfix/*` (off main, merge to main + develop)
...
7. **No force-pushes.** Blocks `git push --force` and `--force-with-lease`. Locks history.
8. **No deletions.** Blocks `git push --delete`. Protects against accidental branch removal.

For GitFlow, you'd apply similar rules to BOTH `main` AND `develop`, and additionally protect `release/*` and `hotfix/*` branches during their active lifecycle.
...
- For releases: tag a specific main commit as `v2.0` (`git tag -a v2.0 -m "Release v2.0"` then `git push origin v2.0`)
...
- For releases: cut a `release/v2.0` branch off main, run QA, merge back when ready
- For hotfixes: same as GitFlow (`hotfix/*` off main)
...
**Pick GitFlow.** Main + develop + `release/*`. Hotfix branches off main when a production crash happens. The release branch is where QA happens. The "two versions in flight" scenario is exactly what GitFlow was designed for.

Setup: protect main + develop + `release/*` + `hotfix/*`. Each requires PR + approvals + CI green. Tag every release commit on main.
...
You have opened **Phase 3** with the foundational workflow taxonomy. L10 covers the release mechanics that all three workflows interact with: tags, semantic versioning, release notes, and the discipline of marking specific commits as releases.
