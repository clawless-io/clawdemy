---
title: 'Lesson: Pull requests'
description: Pull requests are where modern engineering culture lives. You learn the mechanical flow (push branch, open PR, address review, merge), how to write a PR description that respects the reviewer's time, the three merge strategies, and the etiquette of giving and receiving code review.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
2. **Push the branch.** `git push -u origin feature/payment-flow`. The `-u` sets the upstream so future `git push` calls know where to send commits.
...
A new "merge commit" `M` is created on main with two parents: the last main commit (D) and the last feature commit (F). All feature commits are preserved. The merge is explicit in history.
...
- Read your own diff. Catch the typos, the leftover `console.log`s, the things you can fix without a reviewer's eyes. The PR description above is the contract; if you read your own diff and the contract is wrong, fix the contract.
...
- Respond to every comment. Even "good catch, fixed in `<commit-sha>`." Silence on a comment looks like avoidance.
...
A merged PR is a permanent record. Six months from now, someone will git-blame a line of code, find a commit, find the PR linked from the commit message, and read your PR description to understand what was happening.
