...
The shared remote now sees `agent-1/feature` (and similarly each agent's branch).
...
The shared remote (your team's main remote) does NOT see `agent-1/feature`. It lives only on agent-1's fork.
...
**3. Watching the fleet during work.** Periodically run `git log --all --oneline --graph` to see what each agent is committing. Catch agents that are going off-script early (before they pile up a lot of bad work).
...
Agent A modifies `models/user.py`. It adds a new field called `last_login_timestamp`.

Agent B modifies `services/email.py`. It adds a new function that sends an email using fields from the User model, including (in B's understanding of the model) a field called `last_login_time` (note the different name).

Both agents committed. Both branches merged cleanly, they touched different files. Git sees no conflict. Tests run...

If you have good unit tests, B's email function tries to access `user.last_login_time` and gets `AttributeError`. The test fails. Conflict detected, late but catchable.
...
**Layer 1: Tight agent scope.** Reduce overlap. If Agent A is working in `models/`, Agent B in `services/`, and Agent C in `templates/`, the chance of semantic conflict is low (different mental models for different concerns).
...
**Layer 3: The lead reads the integrated diff.** Before pushing to main, the lead reads `git diff main..integration/<feature-name>` and looks for the semantic seams. This is judgment work; no test framework can fully substitute. The lead looks for:
...
Lead scans for other semantic seams. Notices that agent-ui added a `validate_bio()` function and agent-api added a different `validate_bio_field()` function. Both do the same thing. Lead decides to keep the agent-ui version (more thorough) and refactor agent-api to use it.
...
Your repo has a main remote at `origin` (your team's). The external agent has their own remote at `external-agent-1`.
