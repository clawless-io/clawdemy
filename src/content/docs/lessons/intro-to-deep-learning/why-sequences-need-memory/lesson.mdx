---
title: "Lesson: Why sequences need memory"
description: The networks so far took a whole input at once and answered in one shot. But a sentence, a melody, or a stock chart arrives in order, one piece at a time, and the order is the meaning. This lesson shows why a plain feedforward network struggles with ordered data, and how giving a network a memory, a running summary it updates step by step, lets it handle sequences. That memory is the idea behind recurrent networks.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
