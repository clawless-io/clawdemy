---
title: "Lesson: How machines see: convolution"
description: "The digit network from earlier treated all 784 pixels as unrelated inputs, which throws away the most basic fact about an image: nearby pixels form shapes, and a shape can appear anywhere. This lesson introduces the fix, convolution, a small filter that slides across an image looking for a local pattern. We work an edge detector by hand and see why sharing one small filter everywhere is what makes vision networks efficient."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
