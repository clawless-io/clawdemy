...
description: "The opener of Track 12's vision phase. The digit network treated all 784 pixels as unrelated inputs, throwing away the most basic fact about an image: nearby pixels form shapes, and a shape can appear anywhere. This lesson introduces the fix, convolution, a small filter that slides across an image looking for a local pattern. It works an edge detector by hand, shows why sharing one small filter everywhere makes vision networks efficient and translation-invariant, and sets up the stacking that turns edges into objects."
