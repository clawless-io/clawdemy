...
<ReadAlongDim
  src="https://audio.clawdemy.org/lessons/planning-with-models-lesson.mp3"
  slug="planning-with-models"
...
You have a learned dynamics model P-hat and (either learned or given) reward R-hat. You are currently at the current state. You want to choose an action that maximizes expected discounted return:
...
The terminal value V-hat at the horizon state is an estimate of the value beyond the horizon, optionally from a learned critic. If you trust the model only out to about 5 steps, set H = 5; the terminal value patches what the truncation misses.
...
So q-1 is a Gaussian with mean 0.65 and standard deviation 0.15. Iteration 2 samples from this much tighter Gaussian, concentrated near the optimum, a-star equal to 1.0. After 5 iterations, CEM converges to within 0.01 of a-star equal to 1.
...
- **EfficientZero** (Ye et al., 2021): MuZero with self-supervised consistency losses, achieves Atari median human performance from only two hours of real-time game experience (the Atari 100k data budget; wall-clock training time depends on hardware).
