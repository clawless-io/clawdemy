...
- Build the **graphical model** that turns the RL trajectory (s_0, a_0, s_1, a_1, ..., s_T) into a Bayesian inference problem by introducing binary **optimality variables** O_t with p(O_t = 1 | s_t, a_t) ∝ exp(r(s_t, a_t) / alpha). The MaxEnt-RL problem is then "infer the posterior over actions given that O_(1:T) = 1."
...
- Compute soft Q-values and the resulting policy by hand on a small 2-action terminal example. Verify both limits: alpha → 0 recovers the hard Bellman backup; alpha → ∞ recovers the uniform policy.
...
- Identify **RLHF** as the same framework with a different prior: replace the uniform action prior with pi_pretrained(z | prompt), and you get the KL-regularized PPO objective from Lesson 8.
...
Start with a standard Markov decision process: trajectory tau = (s_0, a_0, s_1, a_1, ..., s_T) with prior p(tau) = p(s_0) · Product_t p(a_t | s_t) · p(s_(t+1) | s_t, a_t). The standard action prior p(a_t | s_t) is uniform (call this the "uninformative" prior).

The clever trick: introduce binary **optimality** variables O_t in (0 or 1) for each timestep. Define:

```
p(O_t = 1 | s_t, a_t) = exp(r(s_t, a_t) / α)
```

alpha is a temperature hyperparameter. With this definition, O_t = 1 is more probable when the reward r(s_t, a_t) is high. The trajectory becomes "evidently optimal" if O_(1:T) = 1.

Note: exp(r/alpha) is not normalized as a probability; we can renormalize implicitly later. For now, treat it as an un-normalized likelihood (an "energy" in physics terms).
...
The RL problem: **find the posterior over actions given that all O_t = 1**, i.e., the trajectory was optimal. By Bayes' rule:
...
where R(tau) = Sum_t r(s_t, a_t) is the trajectory return. So the posterior distribution over trajectories is proportional to the prior weighted by the soft-Boltzmann distribution exp(R(tau) / alpha). Higher-return trajectories are more probable; the temperature alpha controls how sharply.

This is the same structure as a Boltzmann distribution over physical states with energy -R(tau): low-energy (high-reward) states are more probable. The temperature alpha plays the role of kT in statistical mechanics.

## Backward message passing

The natural inference question: given the optimality evidence at all future timesteps, what's the probability of each action now? Mathematically: compute p(a_t | s_t, O_(t:T) = 1).
...
where beta(s_t, a_t) = p(O_(t:T) = 1 | s_t, a_t) is the **backward message**. It satisfies a backward recursion:
...
Define Q_soft(s, a) := alpha · log beta(s, a). Then:

```
Q_soft(s_t, a_t) = r(s_t, a_t) + α · log E_{s_{t+1}} [V(s_{t+1})]
```

And V_soft(s) := alpha · log V(s) = alpha · log Sum_a p(a|s) · exp(Q_soft(s, a) / alpha). With uniform p(a|s) = 1/|A|:
...
The constant -alpha · log|A| is the entropy of the uniform prior; it cancels out in the policy improvement step, so most expositions drop it and write the soft value function as:

```
V_soft(s) = α · log Σ_a exp(Q_soft(s, a) / α)
```

Two derivation steps got compressed here that are worth surfacing. The exact backward message at line 107 carries a log-E factor over next states; rewriting line 107 with that exact log-E factor gives the naive exact-inference form Q_soft = r + alpha · log E over s-prime of exp(V_soft(s-prime)/alpha). That form treats the dynamics posterior as if the agent controlled it and is **risk-seeking / optimistic** under stochastic transitions (the "optimism problem" of message passing). The **variational correction** fixes the dynamics posterior to the true p(s-prime given s, a), replacing the log-E factor with the plain expectation gamma · E over s-prime of V_soft(s-prime). The discount gamma is the standard RL discount (classic control-as-inference is undiscounted; mixing gamma here is a pedagogical blend so the backup composes with the rest of the track). With those two corrections, the **soft Bellman backup** has its two recursions:
...
(Subtle point: the axis is **exact-inference vs variational**, not deterministic-vs-stochastic. Naive exact-inference message passing under stochastic transitions yields Q_soft = r + alpha · log E over s-prime of exp(V_soft(s-prime)/alpha), a log-sum-exp over next states that is **risk-seeking / optimistic** under uncertainty (the "optimism problem" of naive message passing). The **variational correction** restores the plain expectation r + gamma · E over s-prime of V_soft(s-prime), which is the form SAC actually uses regardless of whether dynamics are deterministic or stochastic. Levine 2018 makes the axis explicit: exact inference is appropriate for deterministic dynamics, variational inference for stochastic.)
...
(Using V_soft = alpha · log Z from the partition-function definition.) This is a Boltzmann distribution over actions with energies -Q_soft(s, a) and temperature alpha.

## Two limits sanity-check the framework

The temperature alpha interpolates between two known limits.

### Limit alpha → 0 (deterministic / hard Bellman)

alpha · log Sum_a exp(Q/alpha) → max_a Q as alpha → 0 (log-sum-exp converges to max). So:
...
### Limit alpha → ∞ (uniform policy)

As alpha → ∞, the soft-max becomes flat: every action looks roughly equally good. The policy pi_soft → uniform. No information from rewards is being used.

### The sweet spot

Real applications pick alpha in between: SAC tunes alpha automatically (Haarnoja et al., 2018 has the automatic-temperature variant). RLHF uses the analogous beta parameter; current systems pick beta ∈ [0.01, 0.1] typically.
...
Set up the smallest non-trivial case. Single state s, two actions a_1, a_2. Both terminate immediately after one step.
...
Terminal one-step: there is no continuation. So Q_soft(s, a) = r(s, a):
...
- At alpha = 0.01: pi_soft(a_1) = exp(100) / (exp(100) + exp(0)) ≈ 1.0 (greedy).
- At alpha = 100: pi_soft(a_1) = exp(0.01) / (exp(0.01) + exp(0)) = 1.01 / 2.01 ≈ 0.502 (almost uniform).

The dual-path verification: in the alpha → 0 limit, soft policy reduces to greedy (hard Bellman); in the alpha → ∞ limit, soft policy reduces to uniform. The framework matches the known endpoints exactly.
...
- **Soft Q-critic** Q_phi(s, a): trained to minimize the squared error against the soft Bellman target r + gamma E [V_soft(s')].
- **Stochastic actor** the policy parameterized by theta(a | s): trained to minimize KL(the policy parameterized by theta(·|s) || exp(Q_phi(s, ·) / alpha) / Z(s)).

The actor objective is "match the soft posterior implied by the current Q." The critic objective is "regress to the soft Bellman target implied by the current actor and Q." Iteratively, both converge to the variational fixed point Q_soft, pi_soft.

The crucial detail SAC adds: the policy is **reparameterized** (Lesson 11). The action is a = mu_theta(s) + sigma_theta(s) · epsilon with epsilon ~ N(0, I). The KL divergence to the Q-Boltzmann posterior has closed-form gradients through this reparameterization. This is what makes SAC trainable end-to-end.
...
Lesson 8 introduced KL-regularized PPO with objective L = L^CLIP - beta · KL(the policy parameterized by theta || pi_pretrained). The control-as-inference framing recovers this exactly:

- The graphical model has the same O_t optimality variables with p(O_t | x, y) ∝ exp(R(x, y) / beta).
- The prior p(a | s) is **not uniform**; it is pi_pretrained(y | prompt).
- The variational objective J(the policy parameterized by theta) = E_pi[R(x, y)] - beta · KL(the policy parameterized by theta || pi_pretrained) is the ELBO for the RLHF graphical model.

Two minor caveats:

1. RLHF is **sequence-level** rather than per-timestep: the optimality variable is O = 1 for the whole completion, with R(x, y) the cumulative reward model score. The within-completion structure is simpler than full RL because the reward model only fires once per completion.
2. The clipped surrogate L^CLIP from PPO is the **practical optimizer** for the surrogate objective, not part of the variational derivation. The variational framework derives the **objective**; PPO is the **gradient-step machinery** for optimizing it.

Pick a different prior, get a different algorithm. Uniform → SAC. Pretrained → RLHF. Demonstrations → imitation-bootstrapped policy improvement. Each algorithm is the variational solution to the same graphical-model template with different prior choice.
...
- **This lesson (L12)**: the entire RL framework follows from the choice of evidence (O_t = 1), prior (p(a|s)), and temperature (alpha).
...
- **Control as inference** turns RL into Bayesian inference: introduce optimality variables O_t with p(O_t = 1 | s, a) ∝ exp(r(s, a) / alpha); the posterior over trajectories given O_(1:T) = 1 is the MaxEnt-RL distribution.
- **The soft Bellman backup** is V_soft(s) = alpha · log Sum_a exp(Q_soft(s, a) / alpha) and Q_soft(s, a) = r(s, a) + gamma · E_(s')[V_soft(s')]. The log-sum-exp is the "soft max."
- **Two limits**: as alpha → 0, log-sum-exp → max and soft Bellman reduces to hard Bellman (Lesson 6); as alpha → ∞, the policy becomes uniform.
- **Worked example**: 2-action terminal, alpha = 1, r = (1, 0). V_soft = log(e + 1) ≈ 1.313; soft policy (0.731, 0.269). Limits match.
