...
Phase 2 opens the **value branch**. Instead of nudging policy parameters by gradient, you will learn the optimal action-value function Q*(s, a) directly, and then read off the optimal policy as pi-star(s) = argmax_a Q*(s, a). The policy never appears as a separate parameterized object; it falls out of Q*.
...
Recall the Bellman equation from Lesson 3, written for Q^π:
...
This is the value of taking action a in state s, then following policy pi thereafter. It is **linear** in Q^π for a fixed pi, because the inner sum over actions is just an average weighted by pi.

Now consider the **optimal** action-value Q*(s, a), defined as the value of taking action a in state s and then acting optimally forever after. The optimal policy, whatever it is, picks the best action at every future state. So instead of averaging over π's action distribution, you take the max:

```
Q*(s, a) = R(s, a) + γ · E_{s' ~ P(·|s, a)} [ max_a' Q*(s', a') ]
```

This is the **Bellman optimality equation**. The right-hand side has a max inside an expectation, which makes the system non-linear in Q*. There is no closed-form Gaussian-elimination solution like there is for Q^π with a fixed pi. You have to solve it iteratively.

The good news: the operator on the right (call it T*) is a **gamma-contraction** in the sup-norm. For any two functions Q_1 and Q_2:

```
|| T* Q_1 - T* Q_2 ||_∞  ≤  γ · || Q_1 - Q_2 ||_∞
```

By the Banach fixed-point theorem, repeated application converges geometrically to a unique fixed point, which is Q*. That gives you the algorithm.

## Tabular Q-iteration

The cleanest algorithm: start with Q_0 arbitrary (zero is fine), and apply T* over and over:

```
Q_{k+1}(s, a) ← R(s, a) + γ · Σ_{s'} P(s'|s, a) · max_{a'} Q_k(s', a')
```

Iterate over all (s, a) pairs each round. The geometric-contraction bound says || Q_k - Q* ||_∞ ≤ gamma^k · || Q_0 - Q* ||_∞, so error shrinks by a factor of gamma per iteration. For gamma = 0.9, gamma^22 ≈ 0.098 (about an order of magnitude per twenty-two iterations); one decimal place takes roughly 22 iterations, not 10.

### Worked example: the L3 two-state MDP

Lesson 3 used a deterministic two-state loop: s0 → s1 with reward 1, s1 → s0 with reward 0, one action per state, gamma = 0.9. Because there is only one action, Q*(s, ·) = V*(s), so you already know the answer from Lesson 3:
...
Run Q-iteration with Q_0 = 0 everywhere. Since each state has one action, the max is trivial and the update is just Q_(k+1)(s) = R(s) + gamma · Q_k(s'):
...
The geometric ratio between successive errors is gamma squared = 0.81 (because the loop is two steps long; one step picks up the reward, the next bootstraps the value). At iteration 50 the error is still about 0.027; reaching || Q_k - Q* ||_∞ \< 10⁻⁴ takes about 100 iterations. The numbers match Lesson 3's closed-form computation exactly. The same value, derived two ways: this is the dual-path discipline applied to the algorithm itself.
...
By inspection, stay is dominated: it pays a negative reward and lands you back in s0, from which the best you can do is take go for Q*(s0, go) ≈ 5.263. So Q*(s0, stay) = -0.5 + 0.9 · 5.263 ≈ 4.237 \< 5.263. The greedy policy pi-star(s0) = argmax_a Q*(s0, a) = go picks go, as it should.

The lesson: **Q* encodes both the values and the optimal action choice**. You do not need a separate policy network. Just compute Q* and take the argmax.

## From Q-iteration to Q-learning

Q-iteration requires knowing the dynamics P(s'|s, a) and the reward function R(s, a). In real RL you do not; you only observe samples. The fix is to replace the expectation over P with a single sampled next-state. After observing the transition (s_t, a_t, r_t, s_(t+1)):

```
Q(s_t, a_t)  ←  Q(s_t, a_t)  +  α · ( r_t + γ · max_{a'} Q(s_{t+1}, a')  -  Q(s_t, a_t) )
```

This is **Q-learning** (Watkins, 1989). The bracketed quantity is the **TD error** or **Bellman error**: the difference between what you currently estimate Q(s_t, a_t) to be and the new sample-based estimate r_t + gamma · max_(a') Q(s_(t+1), a'). The step size alpha is a learning rate; with appropriate schedules (alpha_t → 0 slowly), tabular Q-learning converges to Q* with probability one under mild ergodicity assumptions.

### Why Q-learning is off-policy by construction

The target r_t + gamma · max_(a') Q(s_(t+1), a') uses the **greedy** policy at s_(t+1) (it picks the best action). But the data collection that produced the transition can use **any** policy, typically an exploring one like epsilon-greedy (with probability epsilon, pick a random action; otherwise, pick the greedy one). The policy that **generated the data** can differ from the policy being **evaluated and improved**. That separation is what off-policy means.

Compare to REINFORCE (Lesson 4): the gradient estimator E_(τ ~ the policy parameterized by theta)[ ∇log the policy parameterized by theta(τ) · R(τ) ] is an expectation under the policy parameterized by theta, the policy you are currently following. Switching policies invalidates the estimator (unless you correct with importance sampling, which is its own can of worms). That is on-policy.
...
Tabular Q-learning works for small discrete state spaces (a 3 by 3 grid world, a tic-tac-toe board). For Atari frames (84 × 84 × 4 pixel stacks), the state space is astronomically large; you cannot store a Q-table. The fix is **function approximation**: parameterize Q as a neural network Q_theta(s, a), and minimize the squared TD error:

```
L(θ) = E_{(s, a, r, s') ~ D} [ ( r + γ · max_{a'} Q_θ(s', a')  -  Q_θ(s, a) )² ]
```

D is a dataset of transitions (a replay buffer in DQN). The minimizer is theta such that Q_theta approximately satisfies the Bellman optimality equation on the support of D.
...
1. **Function approximation.** Q_theta is not a lookup table; it generalizes across states. A weight update for one (s, a) shifts Q_theta at many other (s', a') pairs.
2. **Bootstrapping.** The target r + gamma · max_(a') Q_theta(s', a') uses Q_theta itself, not a true Monte Carlo return. The target moves as you update.
...
Any one of these on its own is safe. Any two are safe. All three together can produce **arbitrarily large divergence**: Q_theta weights blow up and the policy becomes nonsense. This is why naive deep Q-learning (just running gradient descent on the loss above) does not work on Atari.
...
- The **action space is small and discrete**. The max_a is a quick scan. With continuous actions, the argmax is its own optimization problem (this is why most continuous-control algorithms are policy-gradient or actor-critic).
...
- **Confusing the Bellman equation with the Bellman optimality equation.** The first uses the policy's action distribution (linear in Q^π); the second uses max (non-linear). Tabular policy iteration solves the first; tabular Q-iteration solves the second.
- **Calling Q-learning on-policy because the data uses epsilon-greedy.** The policy used for **action selection during data collection** is irrelevant. What makes Q-learning off-policy is that the target uses max_(a'), not E_(a' ~ π). epsilon-greedy is just an exploration mechanism.
- **Assuming Q-iteration converges with function approximation.** The geometric-contraction proof relies on T* being a contraction in sup-norm, which fails when Q_theta generalizes. You can verify this empirically: write a naive deep-Q implementation, train on a toy MDP, and watch the weights diverge.
- **Mistaking the max overestimation bias for noise.** E[max_a X(a)] ≥ max_a E[X(a)] by Jensen's inequality. With noisy Q_theta, the max systematically overestimates. Double Q-learning (Lesson 7) addresses this by using two networks: one picks the action, the other evaluates it.
...
- **The Bellman optimality equation** is Q*(s, a) = R(s, a) + gamma · E_(s')[ max_(a') Q*(s', a') ]. The max makes it non-linear; no closed form, only iterative solutions.
...
- **Q-learning** is the model-free, sample-based version: Q(s, a) ← Q(s, a) + alpha · TD-error, where the TD error uses max_(a') Q(s', a'). The max in the target is what makes Q-learning **off-policy**.
...
- **Greedy policy extraction**: pi-star(s) = argmax_a Q*(s, a). No policy network needed.
