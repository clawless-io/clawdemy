...
This lesson walks through BC, the reason it is structurally tempting, the quantitative reason it fails (errors compound across long trajectories, an O(epsilon T squared) blow-up), and the standard fix called **DAgger** that brings it back down to O(epsilon T). By the end you will see why imitation learning is both the right place to start a deep-RL course *and* the right place to leave.

## The setup: an expert and a dataset

Start with a concrete picture. A human driver demonstrates safe driving for ten hours. Sensors on the car record, at each timestep, the **state** s_t (camera frames, lidar, speed, GPS) and the **action** a_t* the human took (steering angle, throttle, brake). After ten hours at 30 frames per second you have a dataset:

```
D = { (s_1, a_1*), (s_2, a_2*), ..., (s_N, a_N*) }    with N ≈ 1,080,000
```

This is the kind of dataset supervised learning was designed for. Train a neural network the policy parameterized by theta(s) to predict the action given the state, by minimizing a standard supervised loss (cross-entropy for discrete actions, mean squared error for continuous). That trained network is your **policy**: feed it the current state, get back an action.
...
The expert demonstrations come from one distribution of states: the states the *expert* visits while driving well. Call this distribution p_expert(s). The learned policy, no matter how well trained, will make occasional small errors: it steers a touch too left, brakes a touch too late. Each such error puts the car in a slightly different state than the one the expert would have been in. Over time, those small errors compound:
...
The training distribution is p_expert(s); the *test* distribution (the states the policy actually visits) is p_policy(s). These two distributions disagree, and they disagree *more* the longer the trajectory runs. This is called **distribution shift** (or **covariate shift**). In supervised learning you assume training and test data come from the same distribution; in behavioral cloning, the policy you trained *creates* the distribution it is then tested on, and they drift apart.

{/* DIAGRAM PLACEHOLDER: a road from above with the expert's trajectory hugging the lane center as a tight band, and the BC policy's trajectory starting near the center and diverging steadily outward, with the off-lane states marked as "not in training data" */}

## The quantitative bound: O(epsilon T squared) versus O(epsilon T)
...
The simplest algorithm that achieves the O(epsilon T) bound is **DAgger** (Dataset Aggregation), Ross, Gordon, and Bagnell, 2011. The recipe is iterative and intuitive:
...
The crucial difference from BC: the dataset eventually contains states from p_policy(s), not just p_expert(s). The policy learns to recover from its own mistakes, because it now has expert labels at the states it tends to drift into. The compounding stops.
...
**Mistaking a low training loss for a working policy.** A BC policy can achieve near-zero loss on the expert's data and still fail catastrophically in deployment, because near-zero loss on p_expert(s) says nothing about behavior on p_policy(s). Evaluate on rollouts, not on held-out expert-distribution samples.
...
**Underestimating DAgger's cost.** DAgger gives O(epsilon T), but requires the expert to label every batch of policy-visited states. In domains where the expert is a human, that labeling cost can dwarf the original demonstration cost. The practical algorithms most labs use are compromises (interactive correction, mixed BC+RL, learned reward models that approximate the expert's preferences); they all derive from the same recognition that BC's distribution shift needs an answer.

## What you should remember

- **Behavioral cloning is supervised learning on (state, expert action) pairs.** Train a policy the policy parameterized by theta(s) to predict the expert's action by minimizing a standard supervised loss on the expert dataset. No reward, no environment interaction, no exploration during training.
- **BC fails because of distribution shift.** Small per-step errors put the policy in states the expert never visited; the policy was never trained there; it makes bigger errors; it drifts further. The dataset is from p_expert(s); the test distribution is p_policy(s); they disagree, and they disagree more the longer the trajectory runs.
- **The quantitative bound:** BC's expected mistakes scale as O(epsilon T squared) in trajectory length T (quadratic), where epsilon is the per-step error rate. DAgger (which trains on states the policy actually visits, with expert labels) brings this down to O(epsilon T) (linear). At epsilon = 0.01 and T = 200, the worst-case BC bound is 400 mistakes; DAgger's is 2.
