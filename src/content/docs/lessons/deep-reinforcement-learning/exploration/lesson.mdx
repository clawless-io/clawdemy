---
title: "Lesson: Exploration (epsilon-greedy, optimism, intrinsic motivation)"
description: When the agent CAN act in an environment but the reward is sparse or hard to find, exploration becomes the central problem. Three families of strategies (random, optimism-based, intrinsic-motivation) make different trade-offs across simplicity, sample efficiency, and the hardness regime they work in. The hard-exploration regime is the one where random exploration provably fails.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
**Under-tuning the intrinsic-reward weight.** beta in r_total = r_extrinsic + beta · r_intrinsic needs to balance the two. Both extremes fail.
