...
- Compute the **max-overestimation bias** on a small example (three actions, unit-variance noise, true Q = 0) and verify the closed-form result E[max] = 3/(2√π) ≈ 0.846.
...
1. **Function approximation (FA).** A neural network Q_theta(s, a) generalizes. One gradient update for (s, a) shifts Q_theta at many other (s', a') pairs.
2. **Bootstrapping (BS).** The target r + gamma · max_(a') Q_theta(s', a') uses Q_theta itself. The target moves as theta updates.
3. **Off-policy data (OP).** Transitions come from a different distribution than the greedy policy's distribution. In Q-learning this is unavoidable: the target uses max_(a') regardless of how the data was collected.

The three DQN engineering tricks attack one leg each. Look at them in order.

## DQN, the algorithm

The architecture from Mnih et al. (2015): input is a stack of four consecutive 84 × 84 grayscale Atari frames (the four-frame stack makes the input Markov, per the discussion in Lesson 3). Three convolution layers (32 filters 8 × 8 stride 4; 64 filters 4 × 4 stride 2; 64 filters 3 × 3 stride 1), then a fully connected layer of 512 units, then a linear output layer with one Q-value per discrete action (typically 4 to 18 actions per game).
...
- The target uses Q_(theta⁻), a **target network**, with theta⁻ periodically copied from theta but otherwise frozen.
...
1. Take one environment step. Push the new transition (s_t, a_t, r_t, s_(t+1)) into D.
2. Sample a mini-batch of B = 32 transitions uniformly from D.
...
Two costs: memory (a million transitions of 84 × 84 × 4 byte arrays is roughly 7 GB at byte precision) and sample efficiency (early transitions are stale, the policy that generated them no longer matches the policy being learned).

Later refinements: **prioritized replay** (Schaul et al., 2016) samples high-TD-error transitions more often; **n-step replay** stores (s_t, a_t, r_t, ..., r_(t+n), s_(t+n)) for lower-bias bootstrapping. Both are in the "Rainbow" combined-improvements paper.

## Trick 2: target network (patches the BS leg)

If the target r + gamma · max_(a') Q_theta(s', a') uses the same theta you are updating, the loss surface itself moves under your feet. Imagine the cartoon failure mode: you nudge Q_theta(s, a) up by a bit; that nudge increases Q_theta(s', a') at neighboring states because the network generalizes; the new target for the next batch is higher, so the next update pushes Q_theta(s, a) higher still; and so on, into divergence.

The fix is to evaluate the target with a **separate, frozen copy** of the network. Maintain theta⁻, a snapshot of theta taken at the most recent **target update step**. Compute the target with theta⁻. Update theta against this target. Every C gradient steps (DQN 2015 used C = 10,000), copy theta⁻ ← theta.
...
Later refinements: **Polyak averaging** (theta⁻ ← τ theta + (1 − τ) theta⁻ with small τ, used in DDPG and SAC) updates the target network gradually instead of in jumps. Both work; the discrete-copy version was DQN's choice.

## Trick 3: double Q-learning (patches the max overestimation bias)

The third trick fixes a different problem that the first two do not. The max operator in the TD target is biased upward when the Q estimate is noisy. Even if Q_theta is **unbiased** for each individual (s, a), taking the max over actions systematically overestimates Q*.

### Worked example: how big is the bias?

Consider a single state s with three actions and true Q*(s, a) = 0 for all three. The network estimate is noisy: Q̂(s, a) = 0 + epsilon_a with epsilon_a ~ N(0, 1) independent across actions.
...
So even with zero true value and zero-mean noise, the max returns about +0.85 in expectation. With gamma = 0.99 this bias propagates: the next target is also too high, the next-next target is too high, and Q_theta drifts up. The 2015 DQN saw this empirically; van Hasselt et al. (2016) measured it on Atari and showed many games where DQN's learned Q values exceed the maximum possible discounted return for the game.

### The double Q-learning fix

Use **two** value functions, Q_theta (online network) and Q_(theta⁻) (target network). Decouple action **selection** from action **evaluation**:

- Original DQN target: r + gamma · Q_(theta⁻)(s', argmax_(a') Q_(theta⁻)(s', a')) (target net picks and evaluates)
- **Double DQN** target: r + gamma · Q_(theta⁻)(s', argmax_(a') Q_theta(s', a')) (online net picks, target net evaluates)

In the idealized case where the two networks have **independent** noise (call the noise epsilon_theta and epsilon_(theta⁻)):

```
a* = argmax_a Q̂_θ(s', a) = argmax_a ε_{θ,a}
```

This a* is determined by epsilon_theta alone, independent of epsilon_(theta⁻). So the evaluated value is:

```
Q̂_{θ⁻}(s', a*) = 0 + ε_{θ⁻, a*}
```

Because a* is chosen independently of epsilon_(theta⁻), the expectation is E[epsilon_(theta⁻, a*)] = 0. The bias is gone.
...
- **Target network**: a frozen copy Q_(theta⁻) of the online network, used only for computing TD targets, refreshed every C steps. Patches the BS leg by giving the regression a fixed objective for C steps.
- **Double Q-learning**: action selection uses online network Q_theta; action evaluation uses target network Q_(theta⁻). Eliminates the max overestimation bias in the idealized independent case; reduces it substantially in practice. On the three-action toy example with unit-variance noise, the bias drops from ≈ 0.85 to 0.
