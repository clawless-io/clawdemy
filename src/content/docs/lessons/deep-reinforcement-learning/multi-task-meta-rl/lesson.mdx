---
title: "Lesson: Multi-task RL and meta-RL"
description: When an agent has to learn many related tasks, the structure across tasks is usable. Multi-task RL trains one policy on many tasks at once, sharing parameters. Meta-RL trains so the agent can rapidly adapt to a new task with few samples. Three meta-RL families (gradient-based, recurrent, Bayesian) make different design choices about what adaptation looks like.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
