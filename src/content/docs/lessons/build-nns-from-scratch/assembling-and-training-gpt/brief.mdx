...
description: "Lesson 2 of Phase 3 (Building a transformer) in the Build Neural Networks from Scratch track. One self-attention computation is not a transformer; this lesson assembles the full GPT. Several attention heads run in parallel (multi-head attention), paired with a per-token feed-forward layer to form a transformer block; residual connections and layer normalization make a deep stack trainable; positional embeddings give the order-blind model a sense of sequence; and the whole thing, token plus position embeddings feeding a stack of blocks into a softmax head, is trained on next-token cross-entropy. It traces the shapes through a block and shows this is the exact architecture behind every modern chatbot."
