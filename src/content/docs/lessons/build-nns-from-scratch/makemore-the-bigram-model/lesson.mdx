...
description: "A language model assigns probabilities to what comes next. This lesson builds the simplest one from scratch, a character-level bigram model that learns the statistics of names and generates new ones, two ways: by counting letter pairs and normalizing, and as a one-layer neural network trained by gradient descent. The two reach the same answer, and the model is the distant ancestor of every large language model: predict the next token, sample, repeat."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
