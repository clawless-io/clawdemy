...
description: "The autograd engine from the last lesson computes the gradient of any expression. This lesson uses it to do the thing it was built for: assemble neurons into a multilayer perceptron, measure how wrong it is with a single loss number, and train it by gradient descent, nudging every parameter downhill until the loss shrinks. It works one full gradient-descent step by hand and shows that this loop is exactly how every network, GPT included, learns."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
