...
description: "Build self-attention from scratch in a GPT: each token emits a query and key whose match decides who it attends to, with a causal mask blocking the future."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
