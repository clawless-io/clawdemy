...
description: "The opener of Phase 3 (Building a transformer) in the Build Neural Networks from Scratch track. The final phase builds the transformer, and this lesson builds its heart, self-attention, in the GPT setting where each token must gather information from earlier tokens but never peek at later ones. Each token emits a query and a key whose dot product decides who it listens to; a causal mask sets future affinities to negative infinity so softmax gives them zero weight; the output is a weighted sum of the tokens' values. It works a masked attention step by hand and shows that this learned, content-based routing is the mechanism behind every large language model."
