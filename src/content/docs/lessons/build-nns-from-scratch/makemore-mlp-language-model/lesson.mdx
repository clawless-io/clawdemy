...
description: The bigram model saw only one character of context. This lesson extends makemore to a multilayer perceptron that reads several previous characters, using learned character embeddings, the idea from Bengio's 2003 neural language model. Embeddings dissolve the explosion that kills the counting approach and let the model generalize across similar characters, and they are the same mechanism every large language model uses to turn tokens into vectors.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
