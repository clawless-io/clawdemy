...
description: "The flat MLP crammed all of its context into one hidden layer, fusing every character at once. This lesson restructures it into a hierarchy, in the style of DeepMind's WaveNet, that fuses context gradually, two groups at a time, so the span it can see doubles with every layer. It is the principle behind depth itself: stack simple layers and let the network build understanding in stages, the same idea that makes transformers work."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
