...
- To a computer, a digit is not a shape but a **pixel grid**: a grid of cells where each cell holds a **brightness number** for how dark it is. A common setup is 28 by 28, which is **784 numbers** and nothing more. There are no curves or loops anywhere in the data, only the list of numbers.
