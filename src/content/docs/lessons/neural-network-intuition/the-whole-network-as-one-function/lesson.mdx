---
title: "Lesson: The whole network as one function"
description: Three lessons ago we promised a function from 784 numbers to 10. Now we have all the parts to assemble it. This lesson zooms back out and shows the entire network as a single function, evaluates a tiny one end to end by hand, and makes the case that everything the network can do lives in the specific values of its thousands of weights and biases, nothing more.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
Let us run a complete one by hand. A full 784-to-10 network would bury us in arithmetic, so here is a miniature with the same shape: 3 inputs, one hidden layer of 2 neurons, and 2 outputs. We will use ReLU (the `max(0, x)` squish from lesson 3) because it keeps the numbers clean.
...
So the hidden layer's activations are `[0.4, 0.0]`. Notice h2 went completely dark; its weighted sum landed negative and ReLU clamped it to zero. That is normal.

**Output layer.** Two neurons, now reading the hidden layer's `[0.4, 0.0]` as their inputs.
...
Output: `[0.24, 0.0]`. The first output neuron wins, so this little network's answer is "the first class." And that is a full forward pass: we turned an input into an output by doing nothing but the lesson-3 computation, twice per layer, in order. Scale this up to 784 inputs, two hidden layers of 16, and 10 outputs, and it is the identical procedure, just with more multiply-add-squash steps.
...
Mathematicians write this as `f(x; w, b)`: a function `f` whose input is `x` (the 784 pixel values), and which also depends on `w` and `b` (the whole collection of weights and biases). The semicolon is doing real work. It separates the thing that varies per use (`x`) from the thing that defines which network this is (`w` and `b`).

Change `x` and the output changes because you showed it a different picture. Change `w` or `b` and the output changes because you are now holding a different network. That second kind of change is the one to sit with: the architecture (784, 16, 16, 10) is just a skeleton, and the weights and biases are what turn that skeleton into a specific, behaving thing.
...
**Confusing the input with the parameters.** The input (`x`) changes every time you use the network. The weights and biases (`w` and `b`) are fixed and define the network. Mixing these up is the most common confusion in the whole chapter.
...
- **The parameters are the function.** Written `f(x; w, b)`: the input `x` varies per use; the weights and biases `w, b` are fixed and define which network you have. Same skeleton plus different parameters equals different behavior.
...
Next: the cheatsheet puts the forward pass and the `f(x; w, b)` framing on one page. Then lesson 5 takes on the question this whole chapter has been circling. How do you find the right thirteen thousand numbers? That search has a name, and it is called learning.
