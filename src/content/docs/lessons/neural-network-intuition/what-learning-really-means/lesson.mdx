---
title: "Lesson: What learning really means"
description: "Lesson 4 ended on a cliffhanger. A network only works once its thousands of weights and biases are set to the right values, so how do we find them? The answer starts with a scorecard. This lesson defines the cost function, the single number that says how wrong the network currently is, works it by hand on a good and a bad output, and reframes the whole of learning as one clean idea: adjust the knobs to make that number small."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
So we can write the cost as `C(w, b)`: a function whose inputs are `w` (every weight in the network) and `b` (every bias), and whose output is that single wrongness number. For our small digit network, `C` takes about 13,000 numbers in and returns one number out. It is a function from the entire parameter space to a single score.

Sit with how strange and useful that is. In lesson 4 we said the network itself is a function, `f(x; w, b)`, that turns an image into a guess. Now we have a second function stacked on top, `C(w, b)`, that turns a whole setting of the knobs into a grade. The input to the network is an image; the input to the cost is an entire network.

## Learning is just making C small

And now learning is no longer mysterious at all. Training the network means finding values of the weights and biases that make `C(w, b)` as small as possible. That is the entire conceptual goal. Not "teaching the network about threes," not "showing it what digits mean," just: search the space of possible knob settings for one that drives the wrongness number down.
...
- **The cost is a function of the knobs, `C(w, b)`,** mapping the roughly 13,000 weights and biases to a single score. For a fixed training set, only the parameters are free to move.
- **Learning is minimizing `C(w, b)`.** Training is an optimization problem: find the knob settings that make the wrongness number small. No magic, just a number going down.
