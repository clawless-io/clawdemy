...
description: "Check yourself, then sort real falsehoods by type, drill lateral reading on a benign claim, and fact-check a model in Clawless. You verify, never create."
sidebar:
  order: 3
  label: Practice
artifact: practice
...
Three exercises. The first two you can do on paper; the third runs in Clawless.
...
This step runs in Clawless, the working environment we use across Clawdemy, and a plain conversation is all it needs. The goal is to catch a hallucination in the wild and practice treating a model as a subject to verify, never an oracle and never a tool for making anything false.
