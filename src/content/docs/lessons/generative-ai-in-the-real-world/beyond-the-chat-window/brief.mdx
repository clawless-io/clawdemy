...
learning_outcomes:
  ...
  - Build a tailored assistant in Clawless Computer with standing instructions and your own documents, and test where its lane ends
...
**Why this order.** Lesson 2 taught you to assemble task, instructions, and context by hand, one conversation at a time; this lesson makes that anatomy permanent and industrializes the context, so the track's prediction-machine model keeps explaining everything new. The practice in [Clawless Computer](https://clawless.ai) builds the two-capability assistant step by step and finishes with the lane-edge test. Lesson 4 turns capability into judgment: should AI do this task at all, the course's decision framework for that question, and a cautionary tale about an official chatbot that gave the public wrong answers.
