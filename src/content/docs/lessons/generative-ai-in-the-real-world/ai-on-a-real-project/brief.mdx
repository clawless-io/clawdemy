...
learning_outcomes:
  ...
  - Build tailored application materials in a plain Clawless Computer conversation, including the one sentence move that has the machine interview you before it does the task
...
**Why this order.** Lessons 2 through 4 delivered the skills separately, each with its own practice; a case study is how separate skills become one working method. The practice runs the whole method in [Clawless Computer](https://clawless.ai) on your own project: name the role, shape two pieces for two different readers, make one explicit which-pieces call, then rehearse until the interviewer finds a question you cannot answer yet. Lesson 6 opens the track's second movement, from how do I use this well to what it all means for your world, starting with the kinds of risk this technology carries and the levers for managing them.
