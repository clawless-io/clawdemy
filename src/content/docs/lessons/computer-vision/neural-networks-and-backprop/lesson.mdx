---
title: "Lesson: Learning features instead of coding them, neural networks and backprop"
description: Lesson 2 capped us at one template per class. Lesson 3 gave us a training loop. This lesson closes Phase 1 by lifting that cap. We stack linear layers with a non-linearity between them (a neural network), watch the hidden layer learn its own features instead of having them hand-coded, and meet backpropagation, the algorithm that gets the gradient through every weight in every layer in one efficient backward sweep. By the end, you have the full general-purpose image classifier the rest of the track refines.
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
Three lessons have brought us here. Lesson 1 named the data-driven move (stop writing rules, show labeled examples). Lesson 2 gave the simplest concrete machine that carried it out: `s = W · x + b`, one template per class, with a clearly visible ceiling, the famous two-headed horse. Lesson 3 turned that machine into a learner by defining a loss and a step downhill on it. The remaining problem is real: one template per class is not enough for real visual classes that come in many forms. We need a classifier that can hold multiple ways of being a horse and still arrive at "horse."
...
Before adding the trick, see why the obvious move fails. We could try to gain capacity by stacking another linear layer on top of the first: feed `W1 · x + b1` into `W2 · (...) + b2`. Multiply it out:
...
The composition is just another linear layer with `W' = W2 · W1` and `b' = W2 · b1 + b2`. Stacking linears alone gives you exactly the model you started with: one linear classifier in a fancier suit. There is no new representational power. To get more, we need something a linear map cannot do.
...
The hidden layer is where this becomes interesting. The `H` numbers it produces are not pixel values; they are **learned features** of the image. Each hidden unit looks at all 3072 input pixels through its own row of `W1`, computes a weighted sum, and the ReLU keeps the result if it is positive or kills it to zero. The final linear classifier `W2 · h + b2` then sees those `H` numbers, not the raw pixels, and reads off a label.
...
The classical CS231n example makes the mechanics concrete on a tiny circuit. Define `f(x, y, z) = (x + y) * z` and pick `x = -2, y = 5, z = -4`.

**Forward pass.** Compute `q = x + y` and then `f = q * z`.
...
**Backward pass.** We want `df/dx`, `df/dy`, and `df/dz`. The output's gradient on itself is 1, so the backward starts with `df/df = 1`.

The last gate is the multiplier `f = q * z`. Its local gradients are `∂f/∂q = z` and `∂f/∂z = q`. So multiplying by the upstream gradient (which is 1 here):
...
The previous gate is the adder `q = x + y`. Its local gradients are both 1 (`∂q/∂x = 1, ∂q/∂y = 1`). So multiplying by the upstream gradient (`df/dq = -4`):
...
The full gradient is `[df/dx, df/dy, df/dz] = [-4, -4, 3]`. Notice the structure: each gate handled its own local derivative, an upstream number arrived at it, and it sent properly-multiplied gradients to its inputs. The add gate did not know it was inside a `(x+y)*z` expression; it just did its local thing.
...
1. **Forward pass.** Feed a mini-batch of images through the network: `h = ReLU(W1 · x + b1)`, `s = W2 · h + b2`. Get scores.
...
4. **Step.** `W ← W - α * ∇L` for every weight matrix. Move to the next mini-batch.
...
- **Stacking linear layers alone changes nothing** (`W2 · (W1 · x + b1) + b2 = W' · x + b'`). A non-linearity (commonly `ReLU(z) = max(0, z)`) between layers is what gives a neural network its capacity.
- **A two-layer NN: `h = ReLU(W1 · x + b1); s = W2 · h + b2`.** The hidden layer produces `H` **learned features** of the image; the final linear layer maps those features into class scores. This is what broke the one-template-per-class limit.
