...
Object detection takes an image and produces a list of `(class label, bounding box)` pairs, one per detected object. The output is not a single answer; it is a variable-length list, which means the architecture has to do something more than classification.
...
**A worked IoU.** Take two boxes in standard `(x1, y1, x2, y2)` format. Predicted box `A = (0, 0, 10, 10)`, ground-truth box `B = (1, 1, 11, 11)`. Compute the intersection rectangle:
...
**Mask R-CNN (He et al. 2017)** for instance segmentation. Built on top of Faster R-CNN: in addition to predicting `(class, box)` per proposed region, it predicts a per-region binary mask saying which pixels inside the box belong to the object. Conceptually clean: detection plus a per-instance segmentation head.
