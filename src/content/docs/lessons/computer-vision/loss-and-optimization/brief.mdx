...
description: "Lesson 2 left us with a classifier (s = W · x + b) and no way to set its knobs. This lesson defines both halves of the answer. A loss function turns 'predictions match labels' into a single number to drive down (we define multiclass SVM and softmax / cross-entropy and work each on the same worked example); regularization adds a penalty on large weights for better generalization; and gradient descent is the loop that nudges W and b in the negative-gradient direction with step size set by the learning rate. We name analytic vs numerical gradients and mini-batch / SGD as the practical realization. That four-step cycle (forward pass, loss, gradient, step) is how every classifier in this track, including the giants ahead, actually trains."
