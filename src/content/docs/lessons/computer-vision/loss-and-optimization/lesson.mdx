...
We left lesson 2 with a working machine and no idea how to set its dials. We can compute a score per class with `s = W · x + b`, and we can pick the highest. But how do we choose W and b in the first place? A network with the right values is a useful classifier; a network with random values is a noise generator. The training problem is exactly the problem of *finding the right values*, automatically, from labeled examples.
...
where `L_i` is the loss on training image `i`. The only thing left to decide is what `L_i` looks like. Two choices dominate the field, and they are the two CS231n teaches first.
...
Written for one image with correct class `y` and scores `s`, summed over the wrong classes `j ≠ y`:

```
L_i = Σ_{j ≠ y}  max(0, s_j - s_y + 1)
```

Read that carefully. `s_j - s_y` is how much the wrong class `j` outscores the correct one (negative if the correct class is ahead). Adding 1 turns "ahead by 0" into "ahead by 1," so to push the loss to zero the correct class must be ahead by at least 1 from every other class. The `max(0, ...)` ignores classes that already meet the margin.

Let us run last lesson's scores through it. The toy 2-by-2 / 3-class classifier predicted scores `[s_X, s_Y, s_Z] = [0.8, 0.3, -0.4]`, and the correct class is X.
...
This forces the scores into the range `(0, 1)` and makes them sum to 1, so they read as probabilities. The **cross-entropy loss** is then minus the log of the probability the model gave to the correct class:

```
L_i = -log(p_y)
```

When the model is confident and correct, `p_y` is near 1 and `-log(p_y)` is near 0. When the model is wrong or hesitant, `p_y` is small and the loss grows large. Run it on the same scores:
...
The `λ` ("lambda") is a tuning knob that controls how much we care about small weights versus low data loss. Small λ lets the classifier prioritize fitting the data; large λ shrinks W more aggressively. The whole expression on the right is what the optimizer actually drives down.
...
`∇L` is the gradient, a vector the same shape as `W`, pointing steepest uphill in W-space. The minus sign turns it around so we head downhill. `α` is the **learning rate** (sometimes called the step size), a small positive number controlling how far to step.
...
Training takes hours or weeks because it is millions of small downhill steps, not one solved equation. "Convergence" means the loss stopped meaningfully decreasing, which is the optimizer telling you the local terrain has flattened. Two training runs of the same model can land at slightly different solutions because each rolled down its own random starting position into its own local valley. And when you read about a model's "loss curve" in a paper, you are looking at a plot of `L_data + λ * Σ W²` as a function of training steps; the lower it gets, the better the network fits its training data (with the standard caveat that real success is measured on unseen images, not the training loss).
...
**Confusing SVM and softmax losses.** They are different functions that produce different numbers; either can be used to train the same `s = W · x + b` model. Production image classifiers usually use softmax / cross-entropy.

## What you should remember

- **A loss turns "predictions vs labels" into one number** to drive down. Two standards: **multiclass SVM** (`max(0, s_j - s_y + 1)` summed over wrong classes; demands a margin) and **softmax / cross-entropy** (`-log(p_y)` after softmax; demands probability mass on the correct class).
- **Regularization adds a penalty on large weights** (commonly `λ * Σ W²`) so the optimizer prefers simpler W's that generalize better to unseen images.
- **Optimization minimizes that loss.** Random search is bad; gradient descent works. The gradient `∇L` is the steepest-uphill direction; the update rule is `W ← W - α * ∇L`; the **learning rate** `α` is the step size and is one of the most consequential hyperparameters.
