...
The most direct image-based depth recovery is **binocular stereo**. Set up: two cameras side by side, separated by a known baseline b, with the same focal length f (in pixel units). A 3D point projects to image coordinates `x_L` in the left image and `x_R` in the right image. The **disparity** at that point is `d = x_L - x_R` (in pixels).
...
A small numerical example. Suppose the cameras have focal length `f = 500 pixels`, baseline `b = 10 cm`, and a particular 3D point shows a disparity of `d = 50 pixels` between the two images:
...
**Confusing stereo disparity with depth directly.** Disparity is *inversely* related to depth (`Z = f·b/d`); a larger disparity means a closer object, not a farther one. Mixing this up is a common first-week mistake.
...
- **Stereo depth recovery:** `Z = (f · b) / d`, with f focal length, b baseline, d disparity. Worked: `f = 500, b = 10cm, d = 50` → `Z = 100cm`. Closer objects give larger disparity; infinite objects give zero.
