...
<ReadAlongDim
  src="https://audio.clawdemy.org/lessons/diffusion-models-lesson.mp3"
...
1. Sample a training image from your dataset.
2. Sample a random timestep t, uniformly between 1 and T.
3. Sample noise from the standard normal and use a closed-form expression (derivable from the forward process applied t times) to produce the step-t noised image from the clean image in one shot.
4. Pass the noised image and the step number to the network. The network's job is to predict the noise that was added.
5. Loss is the mean squared error between the predicted noise and the true noise.
