---
title: "Lesson: The architectures that cracked vision, AlexNet to ResNet"
description: "Lesson 5 introduced the conv layer. This lesson is the story of how it got stacked, between 2012 and 2015, into the architectures that finally cracked computer vision. We walk AlexNet (the inflection), VGG (depth + uniformity), GoogLeNet (smarter not bigger), and ResNet (skip connections, the unlock that let depth actually work), with their parameter counts and the patterns each landmark established. We close with a folded subsection on what it takes to train these networks at scale: data parallelism, model parallelism, mixed precision, and the modern reality that the architecture choice is now coupled with the training cluster's engineering budget."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
The key idea: instead of having a stack of layers compute a target function `H(x)` directly, have them compute the **residual** `F(x) = H(x) - x`, and recover `H(x) = F(x) + x` by adding the input back to the layer's output through a **skip connection** (also called an identity shortcut). Concretely, a residual block looks like:

```
y = F(x) + x
```

where `F(x)` is whatever the conv layers inside the block compute, and `+ x` is the identity shortcut adding the input straight through.

Why this changed everything: when a deeper block does not need to do anything new, learning `F(x) = 0` is easy (the block just behaves as identity through the shortcut). When the block does need to add something, it learns whatever F is required. Either way, gradients flow back through the shortcut unimpeded, which directly addresses the vanishing-gradient problem that had capped pre-ResNet depth.
...
**Thinking "skip connections" are exotic.** They are an `+ x`. The block's output is `F(x) + x` instead of just `F(x)`. The shocking thing is how much that one addition unlocks, not its complexity.
