...
The filter is "looking for" bright on the right and dark on the left, with the middle column neutral. We slide it across the input with stride 1 and no zero-padding. The output spatial size is `(5 - 3 + 0) / 1 + 1 = 3`, so we get a 3 by 3 feature map.
...
The dot product (sum of elementwise products) is `(-1)(0) + (0)(0) + (1)(1) = 1` for the first row, same for the second, same for the third, totaling **3**.
...
Dot product: `(-1)(0) + (0)(1) + (1)(1) = 1` per row, totaling **3** again. The filter is still positioned over the edge.
...
Dot product: `(-1)(1) + (0)(1) + (1)(1) = 0` per row, totaling **0**. The filter sees a uniformly bright region and produces nothing.
...
where `W` is the input spatial size, `F` is the filter spatial size, `P` is padding, `S` is stride. CS231n's worked example: a 7 by 7 input with a 3 by 3 filter, stride 1, pad 0 gives `(7 - 3 + 0) / 1 + 1 = 5`, so a 5 by 5 output. Same input, stride 2: `(7 - 3 + 0) / 2 + 1 = 3`, a 3 by 3 output.
...
4. **Step.** `W ← W - α * ∇L` for every filter weight.
...
**Thinking output size is a choice.** It is fully determined by the input size and the three hyperparameters: `(W - F + 2P) / S + 1`. Picking S, F, P picks the output size; only certain combinations work.
...
- **Convolution applies a small filter at every spatial position.** Filter spatial size is small (3, 5, 11); depth matches input; the output at each position is a dot product. Many filters per layer give a depth-K output volume; the spatial size follows `(W - F + 2P) / S + 1`.
...
- **Parameter count per conv layer:** `K · (F · F · D_in)` weights plus `K` biases, regardless of input image size. AlexNet's first layer: 34,944 parameters; same number for any input image dimensions.
