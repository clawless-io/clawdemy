...
Phase 2 closed with a working image classifier, an object detector, a segmenter, a video model. All of them required labeled data: every image came with a human-attached `cat` or `car` or `road-pixel`. That assumption is starting to look like a luxury.
...
1. Generate two random augmentations (crops, color jitter, blur, flip): call them `(x, x⁺)`. These are a **positive pair**.
2. Treat all other augmented images in the batch as **negative examples**: `(x, x⁻_1), (x, x⁻_2), ...`.
...
A quick worked similarity. Suppose image A's two augmentations produce feature vectors `a = [1, 0]` and `a⁺ = [0.9, 0.4]`, and a different image B produces `b = [-0.5, 0.8]`. Cosine similarity is `cos(u, v) = (u · v) / (||u|| * ||v||)`:
