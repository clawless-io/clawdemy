---
title: "Lesson: Models that imagine the world, world modeling"
description: "Every vision system so far has been reactive (process the current input, output an answer). World modeling is predictive: given the past, predict the future. A self-driving car needs to predict where a pedestrian will be in two seconds; a robot needs to plan, which needs to forward-simulate possible futures; video-generation systems are world models in disguise. This lesson covers learned world models at vision-context level. We see the pixel-space vs latent-space prediction trade-off (with a parameter-cost calculation), walk the landmark architectures (World Models, Dreamer, MuZero, JEPA, video world models), and tie back to the cross-track work that deepens each thread."
artifact: lesson
sidebar:
  order: 2
  label: Lesson
...
A world model is, in the simplest framing, a function. Given a sequence of past observations `o_1, o_2, ..., o_t` (and possibly actions `a_1, ..., a_t`), predict the next observation `o_{t+1}` (and possibly further into the future). Train it on long sequences of real observations; at inference, **roll it forward** by feeding its own predictions back as input to predict more steps ahead.
...
Suppose a vision system needs to predict 30 frames forward at 224 × 224 RGB resolution. Per-frame pixel count is `224 · 224 · 3 = 150,528`. Predicting 30 future frames in pixel space requires producing `30 · 150,528 = 4,515,840` numbers per rollout.

Now suppose the same system uses a learned latent space of dimension 512. Predicting 30 future *latent* vectors requires producing `30 · 512 = 15,360` numbers per rollout. The ratio:
