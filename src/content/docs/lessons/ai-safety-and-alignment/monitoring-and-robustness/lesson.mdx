...
description: "Lesson 3 of Track 23, opener of Phase 2 (safety and alignment). Hendrycks Ch 3.2 + 3.3 split the deployment-time failure surface into two halves: robustness (the system fails) and monitoring (operators do not notice). The capability is to take an incident report and name which half is the primary failure, which sub-mechanism inside that half, and what would have caught it. Both halves are needed; Swiss-cheese intuition for why one without the other is fragile."
...
{/* COMPONENT PLACEHOLDER (Lead wires at promotion): <SourceLecture url="https://www.aisafetybook.com/textbook/monitoring" title="Introduction to AI Safety, Ethics, and Society, Chapter 3: Single Agent Safety (sections 3.2 Monitoring and 3.3 Robustness)" instructor="Dan Hendrycks (Center for AI Safety)" /> */}
